...
TEST_CASE("AES-128 GCM NIST tag vector", "[crypto][gcm]") {
	...
	const unsigned char plaintext[] = {0xd9, 0x31, 0x32, 0x25, 0xf8, 0x84, 0x06, 0xe5, 0xa5, 0x59, 0x09, 0xc5, 0xaf, 0xf5, 0x26, 0x9a,
	                                   0x86, 0xa7, 0xa9, 0x53, 0x15, 0x34, 0xf7, 0xda, 0x2e, 0x4c, 0x30, 0x3d, 0x8a, 0x31, 0x8a, 0x72,
	                                   0x1c, 0x3c, 0x0c, 0x95, 0x95, 0x68, 0x09, 0x53, 0x2f, 0xcf, 0x0e, 0x24, 0x49, 0xa6, 0xb5, 0x25,
	                                   0xb1, 0x6a, 0xed, 0xf5, 0xaa, 0x0d, 0xe6, 0x57, 0xba, 0x63, 0x7b, 0x39, 0x1a, 0xaf, 0xd2, 0x55};
	const unsigned char expected_tag[16] = {0x4d, 0x5c, 0x2a, 0xf3, 0x27, 0xcd, 0x64, 0xa6, 0x2c, 0xf3, 0x5a, 0xbd, 0x2b, 0xa6, 0xfa, 0xb4};
	...
	for (uint32_t offset = 0U; offset < sizeof(plaintext);) {
		...
		REQUIRE(mbedtls_gcm_update(&ctx, block_size, plaintext + offset, ciphertext.data() + offset) == 0);
		offset += block_size;
...
TEST_CASE("AES-128 GMAC GreenBook HLS5 tag vector", "[crypto][gcm]") {
	...
	const unsigned char auth_key[16] = {0xD0, 0xD1, 0xD2, 0xD3, 0xD4, 0xD5, 0xD6, 0xD7, 0xD8, 0xD9, 0xDA, 0xDB, 0xDC, 0xDD, 0xDE, 0xDF};
	const unsigned char expected_tag[12] = {0x1A, 0x52, 0xFE, 0x7D, 0xD3, 0xE7, 0x27, 0x48, 0x97, 0x3C, 0x1E, 0x28};
	...
	mbedtls_gcm_init(&ctx);
	REQUIRE(mbedtls_gcm_setkey(&ctx, MBEDTLS_CIPHER_ID_AES, key, 128) == 0);
	REQUIRE(mbedtls_gcm_starts(&ctx, MBEDTLS_GCM_ENCRYPT, iv, sizeof(iv), aad.data(), aad.size()) == 0);
	REQUIRE(mbedtls_gcm_finish(&ctx, tag, sizeof(tag)) == 0);
	REQUIRE(std::memcmp(tag, expected_tag, sizeof(expected_tag)) == 0);
	mbedtls_gcm_free(&ctx);
}
