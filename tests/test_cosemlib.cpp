...
TEST_CASE("client service helpers reject null inputs", "[cosemlib][services]") {
	...
	REQUIRE(csm_server_services_execute_handler(reinterpret_cast<csm_db_access_handler>(1), NULL, NULL, &req, &arr) == 0);
	REQUIRE(csm_server_services_execute_handler(reinterpret_cast<csm_db_access_handler>(1), NULL, reinterpret_cast<csm_asso_state *>(1), NULL, &arr) == 0);
...
TEST_CASE("client block response rejects corrupted array bounds", "[cosemlib][services]") {
	uint8_t buf[] = {AXDR_GET_RESPONSE, 0x02U, 0x01U, 0x01U, 0x00U, 0x00U, 0x00U, 0x01U, 0xAAU};
	csm_array arr;
	csm_array_init(&arr, buf, sizeof(buf), sizeof(buf), 0U);
...
TEST_CASE("BER length encoding uses standard short and long forms", "[cosemlib][ber]") {
	...
	const uint8_t expected[] = {0x7F, 0x81, 0x80, 0x81, 0xFF, 0x82, 0x01, 0x00};
...
TEST_CASE("catalog parser handles final line and rejects overlong lines", "[cosemlib][catalog]") {
	...
	const std::string overlong = "catalog:\n"
	                             "  - class_id: 1\n"
	                             "    logical_name: \"" +
	    std::string(150, '1') + "\"\n";
...
TEST_CASE("catalog parser rejects overflowing numeric fields", "[cosemlib][catalog]") {
	const char class_overflow[] = "catalog:\n"
	...
	const char obis_overflow[] = "catalog:\n"
	                             "  - class_id: 8\n"
