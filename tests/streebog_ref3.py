#!/usr/bin/env python3
"""Test both L(S(x)) and S(L(x)) orders with correct C constants."""
...
C = [L_transform(Vec_512(i+1)) for i in range(12)]
...
def streebog_g_LS(h, m, n):
    """L(S(x)) order: XOR C, then S, then L"""
...
def streebog_g_SL(h, m, n):
    """S(L(x)) order: XOR C, then L, then S"""
    k = bytes(a ^ b for a, b in zip(h, n))
    u = bytes(a ^ b for a, b in zip(h, m))
...
def streebog256(data, g_func):
    ...
    zeros = bytes(64)
    h = g_func(h, zeros, bytes(n))
    n_zero = bytes(64)
    h = g_func(h, sigma, n_zero)
    return h[:32]
...
for name, g in [("L(S(x))", streebog_g_LS), ("S(L(x))", streebog_g_SL)]:
    h = streebog256(b'', g)
    print(f"Empty {name}: {''.join(f'{b:02x}' for b in h)}")
    h = streebog256(data, g)
    print(f"HLS9_C {name}: {''.join(f'{b:02x}' for b in h)}")
    print()
