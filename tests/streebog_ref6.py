#!/usr/bin/env python3
"""Full reference with COMPUTED C constants and both orders."""
...
# Compute C with both R directions
C_right = [L_right(Vec_512(i+1)) for i in range(12)]
C_left = [L_left(Vec_512(i+1)) for i in range(12)]
...
configs = [
    ("C_right L(S)", make_g(C_right, True, L_right)),
    ("C_right S(L)", make_g2(C_right, True, L_right)),
    ("C_left L(S)", make_g(C_left, True, L_left)),
    ("C_left S(L)", make_g2(C_left, True, L_left)),
    ("C_right no L", make_g(C_right, False, None)),
    ("C_left no L", make_g(C_left, False, None)),
]

for name, g in configs:
    h = streebog256(b'', g)
    h2 = streebog256(data, g)
    e = "3f5b11e2a8c30975dc351857a5f5593271c4d34499eaff0e8459894c5a896e47"
    c = "4c375b843898b6f0a0744051f74e42f2a944581d46c495e743e97abdcd9d7c58"
    em = ''.join(f'{b:02x}' for b in h)
    cm = ''.join(f'{b:02x}' for b in h2)
    print(f"{name:20s} Empty={em[:16]}... HLS9={cm[:16]}... {'*** EMPTY MATCH' if em==e else ''} {'*** HLS9 MATCH' if cm==c else ''}")
