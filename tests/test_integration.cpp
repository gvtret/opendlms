...
static csm_db_code test_db_access(csm_db_context_t *ctx, csm_array *in, csm_array *out, csm_request *request) {
	(void)ctx;
	if (!in || !out || !request)
		return CSM_ERR_OBJECT_ERROR;

	db_ic_inst_t *inst = NULL;
	if (!db_ic_find(request->db_request.logical_name.class_id, &request->db_request.logical_name.obis, &inst) || !inst) {
...
static csm_db_code explicit_test_db_access(csm_db_context_t *ctx, csm_array *in, csm_array *out, csm_request *request) {
	(void)ctx;
	(void)in;
	(void)request;
	explicit_handler_calls++;
	return (csm_array_write_u8(out, AXDR_TAG_UNSIGNED8) && csm_array_write_u8(out, 0x2AU)) ? CSM_OK : CSM_ERR_OBJECT_ERROR;
}

static csm_db_code poison_global_db_access(csm_db_context_t *ctx, csm_array *in, csm_array *out, csm_request *request) {
	(void)ctx;
	(void)in;
	(void)out;
	(void)request;
...
static void test_stack_setup(void) {
	...
	csm_channel_init(test_channels, NUMBER_OF_CHANNELS, test_asso_states, test_asso_configs, NUMBER_OF_CHANNELS);
...
static void test_build_get(csm_array *pkt, uint8_t invoke_id, uint16_t class_id, const csm_obis_code *obis, uint8_t attr_id) {
...
static void
test_build_set(csm_array *pkt, uint8_t invoke_id, uint16_t class_id, const csm_obis_code *obis, uint8_t attr_id, const uint8_t *data, uint8_t data_len) {
...
static void
test_build_action(csm_array *pkt, uint8_t invoke_id, uint16_t class_id, const csm_obis_code *obis, uint8_t method_id, const uint8_t *data, uint8_t data_len) {
...
static int test_do_get(uint8_t invoke_id, uint16_t class_id, const csm_obis_code *obis, uint8_t attr_id, uint8_t *out_buf, uint32_t out_size) {
	csm_array pkt;
...
static int test_do_set(
    uint8_t invoke_id, uint16_t class_id, const csm_obis_code *obis, uint8_t attr_id, const uint8_t *data, uint8_t data_len, uint8_t *out_buf, uint32_t out_size
) {
	csm_array pkt;
	csm_array_init(&pkt, out_buf, out_size, 0, 0);
...
static int test_do_action(
    uint8_t invoke_id, uint16_t class_id, const csm_obis_code *obis, uint8_t method_id, const uint8_t *data, uint8_t data_len, uint8_t *out_buf,
    uint32_t out_size
) {
	csm_array pkt;
	csm_array_init(&pkt, out_buf, out_size, 0, 0);
...
TEST_CASE("Services_ExplicitHandlerDoesNotUseGlobalDatabase", "[integration][services]") {
	...
	int ret = csm_server_services_execute_handler(explicit_test_db_access, &db_ctx, &state, &request, &pkt);
	REQUIRE(ret > 0);
...
TEST_CASE("Integration_SetClockTime", "[integration][basic]") {
	...
	const uint8_t clock_value[] = {AXDR_TAG_OCTETSTRING, DB_CLOCK_DT_LEN, 0x07, 0xEA, 0x06, 0x06, 0x1B, 0x0C, 0x22, 0x38, 0xFF, 0x00, 0x00, 0x00};

	uint8_t buf[1024];
	int ret = test_do_set(0x01, 8, &obis_clock, 2, clock_value, sizeof(clock_value), buf, sizeof(buf));
	REQUIRE(ret > 0);
	REQUIRE(buf[0] == 0xC5);
	REQUIRE(buf[3] == 0x00);
...
TEST_CASE("Integration_ClockActionsDoNotReturnFakeSuccess", "[integration][basic]") {
	...
	const uint8_t clock_value[] = {AXDR_TAG_OCTETSTRING, DB_CLOCK_DT_LEN, 0x07, 0xEA, 0x06, 0x06, 0x1B, 0x0C, 0x16, 0x38, 0xFF, 0x00, 0x00, 0x00};
	...
	ret = test_do_action(0x02, 8, &obis_clock, 2, NULL, 0, buf, sizeof(buf));
	REQUIRE(ret > 0);
	REQUIRE(buf[0] == 0xC7);
	REQUIRE(buf[3] == 0x00);
	...
	ret = test_do_action(0x04, 8, &obis_clock, 1, NULL, 0, buf, sizeof(buf));
	REQUIRE(ret > 0);
	REQUIRE(buf[0] == 0xC7);
	REQUIRE(buf[3] == 250);
}
...
TEST_CASE("Integration_UtilityTablesLongOctetStringUsesBerLength", "[integration][basic]") {
	...
	uint8_t buf[1024];
	int ret = test_do_set(0x01, 26, &obis_utility, 3, set_data, sizeof(set_data), buf, sizeof(buf));
	REQUIRE(ret > 0);
	REQUIRE(buf[0] == 0xC5);
	REQUIRE(buf[3] == 0x00);
...
TEST_CASE("Integration_CompactDataLongOctetStringUsesBerLength", "[integration][basic]") {
	...
	uint8_t buf[1024];
	int ret = test_do_set(0x01, 62, &obis_compact, 2, set_data, sizeof(set_data), buf, sizeof(buf));
	REQUIRE(ret > 0);
	REQUIRE(buf[0] == 0xC5);
	REQUIRE(buf[3] == 0x00);
...
TEST_CASE("Integration_CompactDataCaptureStoresObjectValue", "[integration][basic]") {
	...
	const uint8_t data_value[] = {AXDR_TAG_UNSIGNED32, 0x00, 0x00, 0x00, 0x2A};
	uint8_t buf[1024];
	int ret = test_do_set(0x01, 1, &obis_data, 2, data_value, sizeof(data_value), buf, sizeof(buf));
	REQUIRE(ret > 0);
	REQUIRE(buf[0] == 0xC5);
	REQUIRE(buf[3] == 0x00);

	const uint8_t capture_objects[] = {
	    AXDR_TAG_ARRAY, 0x01, AXDR_TAG_STRUCTURE, 0x03, AXDR_TAG_UNSIGNED16, 0x00, 0x01, AXDR_TAG_OCTETSTRING, 0x06, 0x00, 0x00, 0x60, 0x01,
	    0x00,           0xFF, AXDR_TAG_UNSIGNED8, 0x02
	};
	ret = test_do_set(0x02, 62, &obis_compact, 3, capture_objects, sizeof(capture_objects), buf, sizeof(buf));
	REQUIRE(ret > 0);
	REQUIRE(buf[0] == 0xC5);
	REQUIRE(buf[3] == 0x00);

	ret = test_do_action(0x03, 62, &obis_compact, 2, NULL, 0, buf, sizeof(buf));
	REQUIRE(ret > 0);
	REQUIRE(buf[0] == 0xC7);
	REQUIRE(buf[3] == 0x00);
...
TEST_CASE("Integration_TableManagerLongOctetStringUsesBerLength", "[integration][basic]") {
	...
	uint8_t buf[1024];
	int ret = test_do_set(0x01, 8200, &obis_table_mgr, 4, set_data, sizeof(set_data), buf, sizeof(buf));
	REQUIRE(ret > 0);
	REQUIRE(buf[0] == 0xC5);
	REQUIRE(buf[3] == 0x00);
...
TEST_CASE("Integration_TableManagerRetrieveRowsValidatesSelector", "[integration][basic]") {
	...
	const uint8_t table_data[] = {AXDR_TAG_OCTETSTRING, 0x04, 0x10, 0x20, 0x30, 0x40};
	uint8_t buf[1024];
	int ret = test_do_set(0x01, 8200, &obis_table_mgr, 4, table_data, sizeof(table_data), buf, sizeof(buf));
	REQUIRE(ret > 0);
	REQUIRE(buf[0] == 0xC5);
	REQUIRE(buf[3] == 0x00);

	const uint8_t valid_selector[] = {AXDR_TAG_STRUCTURE, 0x02, AXDR_TAG_UNSIGNED32, 0x00, 0x00, 0x00, 0x01, AXDR_TAG_UNSIGNED32, 0x00, 0x00, 0x00, 0x02};

	ret = test_do_action(0x02, 8200, &obis_table_mgr, 1, valid_selector, sizeof(valid_selector), buf, sizeof(buf));
	REQUIRE(ret > 0);
	REQUIRE(buf[0] == 0xC7);
	REQUIRE(buf[3] == 0x00);
	REQUIRE(buf[4] == 0x01);
	...
	ret = test_do_action(0x03, 8200, &obis_table_mgr, 1, bad_selector, sizeof(bad_selector), buf, sizeof(buf));
	REQUIRE(ret > 0);
	REQUIRE(buf[0] == 0xC7);
	REQUIRE(buf[3] == 250);

	const uint8_t out_of_range_selector[] = {
	    AXDR_TAG_STRUCTURE, 0x02, AXDR_TAG_UNSIGNED32, 0x00, 0x00, 0x00, 0x03, AXDR_TAG_UNSIGNED32, 0x00, 0x00, 0x00, 0x02
	};
	ret = test_do_action(0x04, 8200, &obis_table_mgr, 1, out_of_range_selector, sizeof(out_of_range_selector), buf, sizeof(buf));
	REQUIRE(ret > 0);
	REQUIRE(buf[0] == 0xC7);
...
TEST_CASE("Integration_ProfileFilterRetrieveRowsValidatesSelector", "[integration][basic]") {
	...
	const uint8_t data_value[] = {AXDR_TAG_UNSIGNED32, 0x12, 0x34, 0x56, 0x78};
	uint8_t buf[1024];
	int ret = test_do_set(0x01, 1, &obis_data, 2, data_value, sizeof(data_value), buf, sizeof(buf));
	REQUIRE(ret > 0);
	REQUIRE(buf[0] == 0xC5);
	...
	ret = test_do_set(0x02, 31, &obis_prof_filter, 3, filter_list, sizeof(filter_list), buf, sizeof(buf));
	REQUIRE(ret > 0);
	REQUIRE(buf[0] == 0xC5);
	REQUIRE(buf[3] == 0x00);

	const uint8_t valid_selector[] = {AXDR_TAG_STRUCTURE, 0x02, AXDR_TAG_UNSIGNED32, 0x00, 0x00, 0x00, 0x00, AXDR_TAG_UNSIGNED32, 0x00, 0x00, 0x00, 0x01};

	ret = test_do_action(0x03, 31, &obis_prof_filter, 1, valid_selector, sizeof(valid_selector), buf, sizeof(buf));
	REQUIRE(ret > 0);
	REQUIRE(buf[0] == 0xC7);
	REQUIRE(buf[3] == 0x00);
	REQUIRE(buf[4] == 0x01);
	...
	const uint8_t bad_selector[] = {AXDR_TAG_STRUCTURE, 0x01, AXDR_TAG_UNSIGNED8, 0x00};
	ret = test_do_action(0x04, 31, &obis_prof_filter, 1, bad_selector, sizeof(bad_selector), buf, sizeof(buf));
	REQUIRE(ret > 0);
	REQUIRE(buf[0] == 0xC7);
	REQUIRE(buf[3] == 250);

	const uint8_t out_of_range_selector[] = {
	    AXDR_TAG_STRUCTURE, 0x02, AXDR_TAG_UNSIGNED32, 0x00, 0x00, 0x00, 0x01, AXDR_TAG_UNSIGNED32, 0x00, 0x00, 0x00, 0x01
	};
	ret = test_do_action(0x05, 31, &obis_prof_filter, 1, out_of_range_selector, sizeof(out_of_range_selector), buf, sizeof(buf));
	REQUIRE(ret > 0);
	REQUIRE(buf[0] == 0xC7);
	REQUIRE(buf[3] == CSM_ACTION_RESULT_OTHER_REASON);
}
...
TEST_CASE("Integration_RegisterActivationAddRegisterMutatesObjectList", "[integration][basic]") {
	...
	const uint8_t action_data[] = {AXDR_TAG_STRUCTURE, 0x03, AXDR_TAG_UNSIGNED16, 0x00, 0x03, AXDR_TAG_OCTETSTRING, 0x06, 0x00, 0x00, 0x0A, 0x00, 0x00, 0xFF,
	                               AXDR_TAG_UNSIGNED8, 0x02};

	uint8_t buf[1024];
	int ret = test_do_action(0x01, 6, &obis_reg_act, 1, action_data, sizeof(action_data), buf, sizeof(buf));
	REQUIRE(ret > 0);
	REQUIRE(buf[0] == 0xC7);
	REQUIRE(buf[3] == 0x00);
...
TEST_CASE("Integration_RegisterActivationAddMaskMutatesMaskList", "[integration][basic]") {
	...
	uint8_t buf[1024];
	int ret = test_do_action(0x01, 6, &obis_reg_act, 2, action_data, sizeof(action_data), buf, sizeof(buf));
	REQUIRE(ret > 0);
	REQUIRE(buf[0] == 0xC7);
	REQUIRE(buf[3] == 0x00);
	...
	const uint8_t missing_mask[] = {AXDR_TAG_UNSIGNED16, 0x99, 0x99};
	ret = test_do_action(0x03, 6, &obis_reg_act, 3, missing_mask, sizeof(missing_mask), buf, sizeof(buf));
	REQUIRE(ret > 0);
	REQUIRE(buf[0] == 0xC7);
	REQUIRE(buf[3] == CSM_ACTION_RESULT_OTHER_REASON);
...
TEST_CASE("Integration_ParameterMonitorAddEntryMutatesList", "[integration][basic]") {
	...
	uint8_t buf[1024];
	int ret = test_do_action(0x01, 65, &obis_param_mon, 1, action_data, sizeof(action_data), buf, sizeof(buf));
	REQUIRE(ret > 0);
	REQUIRE(buf[0] == 0xC7);
	REQUIRE(buf[3] == 0x00);
	...
	const uint8_t missing_entry[] = {AXDR_TAG_UNSIGNED16, 0x00, 0x05};
	ret = test_do_action(0x03, 65, &obis_param_mon, 2, missing_entry, sizeof(missing_entry), buf, sizeof(buf));
	REQUIRE(ret > 0);
	REQUIRE(buf[0] == 0xC7);
	REQUIRE(buf[3] == CSM_ACTION_RESULT_OTHER_REASON);
...
TEST_CASE("Integration_ScheduleInsertDeleteMutatesEntries", "[integration][basic]") {
	...
	const uint8_t entry[] = {AXDR_TAG_STRUCTURE, 0x02, AXDR_TAG_UNSIGNED16, 0x00, 0x07, AXDR_TAG_BOOLEAN, 0x01};

	uint8_t buf[1024];
	int ret = test_do_action(0x01, 10, &obis_schedule, 3, entry, sizeof(entry), buf, sizeof(buf));
	REQUIRE(ret > 0);
	REQUIRE(buf[0] == 0xC7);
	REQUIRE(buf[3] == 0x00);
	...
	const uint8_t replacement_entry[] = {AXDR_TAG_STRUCTURE, 0x02, AXDR_TAG_UNSIGNED16, 0x00, 0x07, AXDR_TAG_BOOLEAN, 0x00};
	ret = test_do_action(0x03, 10, &obis_schedule, 3, replacement_entry, sizeof(replacement_entry), buf, sizeof(buf));
	REQUIRE(ret > 0);
	REQUIRE(buf[0] == 0xC7);
	REQUIRE(buf[3] == 0x00);
	...
	const uint8_t missing_entry[] = {AXDR_TAG_UNSIGNED16, 0x00, 0x09};
	ret = test_do_action(0x05, 10, &obis_schedule, 4, missing_entry, sizeof(missing_entry), buf, sizeof(buf));
	REQUIRE(ret > 0);
	REQUIRE(buf[0] == 0xC7);
	REQUIRE(buf[3] == CSM_ACTION_RESULT_OTHER_REASON);

	const uint8_t delete_entry[] = {AXDR_TAG_UNSIGNED16, 0x00, 0x07};
	ret = test_do_action(0x06, 10, &obis_schedule, 4, delete_entry, sizeof(delete_entry), buf, sizeof(buf));
	REQUIRE(ret > 0);
	REQUIRE(buf[0] == 0xC7);
	REQUIRE(buf[3] == 0x00);
...
TEST_CASE("Integration_SpecialDaysInsertDeleteMutatesEntries", "[integration][basic]") {
	...
	const uint8_t entry[] = {AXDR_TAG_STRUCTURE, 0x02, AXDR_TAG_UNSIGNED16, 0x00, 0x11, AXDR_TAG_OCTETSTRING, 0x05, 0x07, 0xEA, 0x06, 0x1B, 0xFF};

	uint8_t buf[1024];
	int ret = test_do_action(0x01, 11, &obis_special_day, 1, entry, sizeof(entry), buf, sizeof(buf));
	REQUIRE(ret > 0);
	REQUIRE(buf[0] == 0xC7);
	REQUIRE(buf[3] == 0x00);
	...
	const uint8_t replacement_entry[] = {AXDR_TAG_STRUCTURE, 0x02, AXDR_TAG_UNSIGNED16, 0x00, 0x11, AXDR_TAG_OCTETSTRING, 0x05, 0x07, 0xEB, 0x01, 0x02, 0xFF};
	ret = test_do_action(0x03, 11, &obis_special_day, 1, replacement_entry, sizeof(replacement_entry), buf, sizeof(buf));
	REQUIRE(ret > 0);
	REQUIRE(buf[0] == 0xC7);
	REQUIRE(buf[3] == 0x00);
	...
	const uint8_t missing_day[] = {AXDR_TAG_UNSIGNED16, 0x00, 0x22};
	ret = test_do_action(0x05, 11, &obis_special_day, 2, missing_day, sizeof(missing_day), buf, sizeof(buf));
	REQUIRE(ret > 0);
	REQUIRE(buf[0] == 0xC7);
	REQUIRE(buf[3] == CSM_ACTION_RESULT_OTHER_REASON);

	const uint8_t delete_day[] = {AXDR_TAG_UNSIGNED16, 0x00, 0x11};
	ret = test_do_action(0x06, 11, &obis_special_day, 2, delete_day, sizeof(delete_day), buf, sizeof(buf));
	REQUIRE(ret > 0);
	REQUIRE(buf[0] == 0xC7);
	REQUIRE(buf[3] == 0x00);
...
TEST_CASE("Integration_ScriptTableExecuteMissingScriptFails", "[integration][basic]") {
	...
	const uint8_t script_id[] = {AXDR_TAG_UNSIGNED16, 0x00, 0x01};

	uint8_t buf[1024];
	int ret = test_do_action(0x01, 9, &obis_script, 1, script_id, sizeof(script_id), buf, sizeof(buf));
	REQUIRE(ret > 0);
	REQUIRE(buf[0] == 0xC7);
	REQUIRE(buf[3] == CSM_ACTION_RESULT_OTHER_REASON);
}
...
TEST_CASE("Integration_SensorManagerResetClearsState", "[integration][basic]") {
	...
	const uint8_t active_variant[] = {AXDR_TAG_UNSIGNED32, 0x00, 0x00, 0x00, 0x2A};
	const uint8_t retries[] = {AXDR_TAG_UNSIGNED8, 0x05};
	const uint8_t protected_flag[] = {AXDR_TAG_BOOLEAN, 0x01};

	uint8_t buf[1024];
	int ret = test_do_set(0x01, 67, &obis_sensor_mgr, 2, active_variant, sizeof(active_variant), buf, sizeof(buf));
	REQUIRE(ret > 0);
	REQUIRE(buf[0] == 0xC5);
	REQUIRE(buf[3] == 0x00);

	ret = test_do_set(0x02, 67, &obis_sensor_mgr, 5, retries, sizeof(retries), buf, sizeof(buf));
	REQUIRE(ret > 0);
	REQUIRE(buf[0] == 0xC5);
	REQUIRE(buf[3] == 0x00);

	ret = test_do_set(0x03, 67, &obis_sensor_mgr, 11, protected_flag, sizeof(protected_flag), buf, sizeof(buf));
	REQUIRE(ret > 0);
	REQUIRE(buf[0] == 0xC5);
	REQUIRE(buf[3] == 0x00);

	ret = test_do_action(0x04, 67, &obis_sensor_mgr, 1, NULL, 0, buf, sizeof(buf));
	REQUIRE(ret > 0);
	REQUIRE(buf[0] == 0xC7);
	REQUIRE(buf[3] == 0x00);
...
TEST_CASE("Integration_RegisterTableCaptureRequiresReadableTarget", "[integration][basic]") {
	...
	uint8_t buf[1024];
	int ret = test_do_action(0x01, 61, &obis_reg_table, 2, NULL, 0, buf, sizeof(buf));
	REQUIRE(ret > 0);
	REQUIRE(buf[0] == 0xC7);
	REQUIRE(buf[3] == CSM_ACTION_RESULT_OTHER_REASON);

	const uint8_t class_id[] = {AXDR_TAG_UNSIGNED16, 0x00, 0x03};
	ret = test_do_set(0x02, 61, &obis_reg_table, 3, class_id, sizeof(class_id), buf, sizeof(buf));
	REQUIRE(ret > 0);
	REQUIRE(buf[0] == 0xC5);
	REQUIRE(buf[3] == 0x00);

	const uint8_t entries[] = {AXDR_TAG_ARRAY, 0x01, AXDR_TAG_STRUCTURE, 0x02, AXDR_TAG_OCTETSTRING, 0x06, 0x00, 0x00, 0x0A, 0x00,
	                           0x00,           0xFF, AXDR_TAG_UNSIGNED8, 0x02};
	ret = test_do_set(0x03, 61, &obis_reg_table, 2, entries, sizeof(entries), buf, sizeof(buf));
	REQUIRE(ret > 0);
	REQUIRE(buf[0] == 0xC5);
	REQUIRE(buf[3] == 0x00);

	ret = test_do_action(0x04, 61, &obis_reg_table, 2, NULL, 0, buf, sizeof(buf));
	REQUIRE(ret > 0);
	REQUIRE(buf[0] == 0xC7);
	REQUIRE(buf[3] == 0x00);

	const uint8_t bad_active_index[] = {AXDR_TAG_UNSIGNED16, 0x00, 0x01};
	ret = test_do_set(0x05, 61, &obis_reg_table, 4, bad_active_index, sizeof(bad_active_index), buf, sizeof(buf));
	REQUIRE(ret > 0);
	REQUIRE(buf[0] == 0xC5);
	REQUIRE(buf[3] == 0x00);

	ret = test_do_action(0x06, 61, &obis_reg_table, 2, NULL, 0, buf, sizeof(buf));
	REQUIRE(ret > 0);
	REQUIRE(buf[0] == 0xC7);
	REQUIRE(buf[3] == CSM_ACTION_RESULT_OTHER_REASON);
}
...
TEST_CASE("Integration_SapAssignmentConnectRequiresAssignedSap", "[integration][basic]") {
	...
	const uint8_t sap_1[] = {AXDR_TAG_UNSIGNED16, 0x00, 0x01};

	uint8_t buf[1024];
	int ret = test_do_action(0x01, 17, &obis_sap, 1, sap_1, sizeof(sap_1), buf, sizeof(buf));
	REQUIRE(ret > 0);
	REQUIRE(buf[0] == 0xC7);
	REQUIRE(buf[3] == CSM_ACTION_RESULT_OBJECT_UNDEFINED);

	const uint8_t sap_list[] = {
	    AXDR_TAG_ARRAY, 0x01, AXDR_TAG_STRUCTURE, 0x02, AXDR_TAG_UNSIGNED16, 0x00, 0x01, AXDR_TAG_OCTETSTRING, 0x06, 'p', 'u', 'b', 'l', 'i', 'c'
	};
	ret = test_do_set(0x02, 17, &obis_sap, 2, sap_list, sizeof(sap_list), buf, sizeof(buf));
	REQUIRE(ret > 0);
	REQUIRE(buf[0] == 0xC5);
	REQUIRE(buf[3] == 0x00);

	ret = test_do_action(0x03, 17, &obis_sap, 1, sap_1, sizeof(sap_1), buf, sizeof(buf));
	REQUIRE(ret > 0);
	REQUIRE(buf[0] == 0xC7);
	REQUIRE(buf[3] == 0x00);
}
...
TEST_CASE("Integration_ActivityCalendarActivatesPassiveCalendar", "[integration][basic]") {
	...
	const uint8_t passive_calendar[] = {AXDR_TAG_OCTETSTRING, 0x06, 's', 'u', 'm', 'm', 'e', 'r'};

	uint8_t buf[1024];
	int ret = test_do_set(0x01, 20, &obis_activity, 2, passive_calendar, sizeof(passive_calendar), buf, sizeof(buf));
	REQUIRE(ret > 0);
	REQUIRE(buf[0] == 0xC5);
	REQUIRE(buf[3] == 0x00);

	const uint8_t malformed_calendar[] = {AXDR_TAG_OCTETSTRING, 0x06, 'w', 'i', 'n', 't', 'e', 'r', 0x00};
	ret = test_do_set(0x02, 20, &obis_activity, 2, malformed_calendar, sizeof(malformed_calendar), buf, sizeof(buf));
	REQUIRE(ret > 0);
	REQUIRE(buf[0] == 0xC5);
	REQUIRE(buf[3] != 0x00);

	ret = test_do_action(0x03, 20, &obis_activity, 1, NULL, 0, buf, sizeof(buf));
	REQUIRE(ret > 0);
	REQUIRE(buf[0] == 0xC7);
	REQUIRE(buf[3] == 0x00);
...
TEST_CASE("Integration_CommsSetupRejectsTrailingSetPayloads", "[integration][basic]") {
	...
	uint8_t buf[1024];
	int ret = test_do_set(0x01, 19, &obis_iec_local, 2, comm_speed, sizeof(comm_speed), buf, sizeof(buf));
	REQUIRE(ret > 0);
	REQUIRE(buf[0] == 0xC5);
	REQUIRE(buf[3] == 0x00);

	ret = test_do_set(0x02, 19, &obis_iec_local, 2, bad_comm_speed, sizeof(bad_comm_speed), buf, sizeof(buf));
	REQUIRE(ret > 0);
	REQUIRE(buf[0] == 0xC5);
	REQUIRE(buf[3] != 0x00);

	ret = test_do_set(0x03, 23, &obis_iec_hdlc, 2, channel, sizeof(channel), buf, sizeof(buf));
	REQUIRE(ret > 0);
	REQUIRE(buf[0] == 0xC5);
	REQUIRE(buf[3] == 0x00);

	ret = test_do_set(0x04, 23, &obis_iec_hdlc, 2, bad_channel, sizeof(bad_channel), buf, sizeof(buf));
	REQUIRE(ret > 0);
	REQUIRE(buf[0] == 0xC5);
	REQUIRE(buf[3] != 0x00);

	ret = test_do_set(0x05, 41, &obis_tcp_udp, 2, ip, sizeof(ip), buf, sizeof(buf));
	REQUIRE(ret > 0);
	REQUIRE(buf[0] == 0xC5);
	REQUIRE(buf[3] == 0x00);

	ret = test_do_set(0x06, 41, &obis_tcp_udp, 2, bad_ip, sizeof(bad_ip), buf, sizeof(buf));
	REQUIRE(ret > 0);
	REQUIRE(buf[0] == 0xC5);
	REQUIRE(buf[3] != 0x00);

	ret = test_do_set(0x07, 41, &obis_tcp_udp, 5, use_dns, sizeof(use_dns), buf, sizeof(buf));
	REQUIRE(ret > 0);
	REQUIRE(buf[0] == 0xC5);
	REQUIRE(buf[3] == 0x00);

	ret = test_do_set(0x08, 41, &obis_tcp_udp, 5, bad_use_dns, sizeof(bad_use_dns), buf, sizeof(buf));
	REQUIRE(ret > 0);
	REQUIRE(buf[0] == 0xC5);
	REQUIRE(buf[3] != 0x00);
...
TEST_CASE("Integration_RegisterMonitorSetAndResetState", "[integration][basic]") {
	...
	const uint8_t thresholds[] = {AXDR_TAG_ARRAY, 0x02, AXDR_TAG_UNSIGNED16, 0x00, 0x64, AXDR_TAG_UNSIGNED16, 0x00, 0xC8};
	const uint8_t monitored[] = {AXDR_TAG_UNSIGNED32, 0x00, 0x00, 0x00, 0x2A};
	const uint8_t actions[] = {AXDR_TAG_ARRAY, 0x01, AXDR_TAG_NULL};

	uint8_t buf[1024];
	int ret = test_do_set(0x01, 21, &obis_reg_monitor, 2, thresholds, sizeof(thresholds), buf, sizeof(buf));
	REQUIRE(ret > 0);
	REQUIRE(buf[0] == 0xC5);
	REQUIRE(buf[3] == 0x00);

	ret = test_do_set(0x02, 21, &obis_reg_monitor, 3, monitored, sizeof(monitored), buf, sizeof(buf));
	REQUIRE(ret > 0);
	REQUIRE(buf[0] == 0xC5);
	REQUIRE(buf[3] == 0x00);

	ret = test_do_set(0x03, 21, &obis_reg_monitor, 4, actions, sizeof(actions), buf, sizeof(buf));
	REQUIRE(ret > 0);
	REQUIRE(buf[0] == 0xC5);
	REQUIRE(buf[3] == 0x00);
	...
	ret = test_do_action(0x06, 21, &obis_reg_monitor, 1, NULL, 0, buf, sizeof(buf));
	REQUIRE(ret > 0);
	REQUIRE(buf[0] == 0xC7);
	REQUIRE(buf[3] == 0x00);
...
TEST_CASE("Integration_SingleActionScheduleSetGetState", "[integration][basic]") {
	...
	const uint8_t executed_script[] = {
	    AXDR_TAG_STRUCTURE, 0x02, AXDR_TAG_OCTETSTRING, 0x06, 0x00, 0x00, 0x09, 0x00, 0x00, 0xFF, AXDR_TAG_UNSIGNED16, 0x00, 0x01
	};
	const uint8_t schedule_type[] = {AXDR_TAG_ENUM, 0x01};
	const uint8_t execution_time[] = {AXDR_TAG_OCTETSTRING, 0x0C, 0x07, 0xEA, 0x06, 0x1B, 0x0C, 0x00, 0x00, 0x00, 0xFF, 0x80, 0x00, 0x00};

	uint8_t buf[1024];
	int ret = test_do_set(0x01, 22, &obis_single_act, 2, executed_script, sizeof(executed_script), buf, sizeof(buf));
	REQUIRE(ret > 0);
	REQUIRE(buf[0] == 0xC5);
	REQUIRE(buf[3] == 0x00);

	ret = test_do_set(0x02, 22, &obis_single_act, 3, schedule_type, sizeof(schedule_type), buf, sizeof(buf));
	REQUIRE(ret > 0);
	REQUIRE(buf[0] == 0xC5);
	REQUIRE(buf[3] == 0x00);

	ret = test_do_set(0x03, 22, &obis_single_act, 4, execution_time, sizeof(execution_time), buf, sizeof(buf));
	REQUIRE(ret > 0);
	REQUIRE(buf[0] == 0xC5);
	REQUIRE(buf[3] == 0x00);
	...
	const uint8_t malformed_type[] = {AXDR_TAG_ENUM, 0x02, 0x00};
	ret = test_do_set(0x06, 22, &obis_single_act, 3, malformed_type, sizeof(malformed_type), buf, sizeof(buf));
	REQUIRE(ret > 0);
	REQUIRE(buf[0] == 0xC5);
	REQUIRE(buf[3] != 0x00);
...
TEST_CASE("Integration_ArbitratorUnsupportedRequestActionFails", "[integration][basic]") {
	...
	const uint8_t action_data[] = {AXDR_TAG_STRUCTURE, 0x01, AXDR_TAG_UNSIGNED8, 0x01};

	uint8_t buf[1024];
	int ret = test_do_action(0x01, 68, &obis_arbitrator, 1, action_data, sizeof(action_data), buf, sizeof(buf));
	REQUIRE(ret > 0);
	REQUIRE(buf[0] == 0xC7);
	REQUIRE(buf[3] == 0x04);
}
...
TEST_CASE("Integration_ImageTransferInitParsesStructure", "[integration][basic]") {
	...
	const uint8_t block_size[] = {AXDR_TAG_UNSIGNED32, 0x00, 0x00, 0x00, 0x32};
	uint8_t buf[1024];
	int ret = test_do_set(0x01, 18, &obis_image, 4, block_size, sizeof(block_size), buf, sizeof(buf));
	REQUIRE(ret > 0);
	REQUIRE(buf[0] == 0xC5);
	REQUIRE(buf[3] == 0x00);

	const uint8_t init_data[] = {AXDR_TAG_STRUCTURE, 0x02, AXDR_TAG_OCTETSTRING, 0x02, 'f', 'w', AXDR_TAG_UNSIGNED32, 0x00, 0x00, 0x00, 0x64};
	ret = test_do_action(0x02, 18, &obis_image, 2, init_data, sizeof(init_data), buf, sizeof(buf));
	REQUIRE(ret > 0);
	REQUIRE(buf[0] == 0xC7);
	REQUIRE(buf[3] == 0x00);
...
TEST_CASE("Integration_ImageTransferBlockTransferUpdatesProgress", "[integration][basic]") {
	...
	const uint8_t block_data[] = {AXDR_TAG_STRUCTURE, 0x02, AXDR_TAG_UNSIGNED32, 0x00, 0x00, 0x00, 0x00, AXDR_TAG_OCTETSTRING, 0x04, 0xDE, 0xAD, 0xBE, 0xEF};
	ret = test_do_action(0x03, 18, &obis_image, 1, block_data, sizeof(block_data), buf, sizeof(buf));
	REQUIRE(ret > 0);
	REQUIRE(buf[0] == 0xC7);
	REQUIRE(buf[3] == 0x00);
	...
	ret = test_do_action(0x07, 18, &obis_image, 1, bad_block, sizeof(bad_block), buf, sizeof(buf));
	REQUIRE(ret > 0);
	REQUIRE(buf[0] == 0xC7);
	REQUIRE(buf[3] == 250);
}
...
TEST_CASE("Integration_ImageTransferRejectsOutOfSequenceActions", "[integration][basic]") {
	...
	const uint8_t verify_data[] = {AXDR_TAG_STRUCTURE, 0x00};
	uint8_t buf[1024];
	int ret = test_do_action(0x01, 18, &obis_image, 5, verify_data, sizeof(verify_data), buf, sizeof(buf));
	REQUIRE(ret > 0);
	REQUIRE(buf[0] == 0xC7);
	...
	const uint8_t init_data[] = {AXDR_TAG_STRUCTURE, 0x02, AXDR_TAG_OCTETSTRING, 0x02, 'f', 'w', AXDR_TAG_UNSIGNED32, 0x00, 0x00, 0x00, 0x04};
	ret = test_do_action(0x03, 18, &obis_image, 2, init_data, sizeof(init_data), buf, sizeof(buf));
	REQUIRE(ret > 0);
	REQUIRE(buf[0] == 0xC7);
	REQUIRE(buf[3] == 0x00);

	ret = test_do_action(0x04, 18, &obis_image, 5, verify_data, sizeof(verify_data), buf, sizeof(buf));
	REQUIRE(ret > 0);
	REQUIRE(buf[0] == 0xC7);
	REQUIRE(buf[3] == CSM_ACTION_RESULT_TEMPORARY_FAILURE);

	ret = test_do_action(0x05, 18, &obis_image, 2, init_data, sizeof(init_data), buf, sizeof(buf));
	REQUIRE(ret > 0);
	REQUIRE(buf[0] == 0xC7);
	REQUIRE(buf[3] == 0x00);
	...
	const uint8_t block_data[] = {AXDR_TAG_STRUCTURE, 0x02, AXDR_TAG_UNSIGNED32, 0x00, 0x00, 0x00, 0x00, AXDR_TAG_OCTETSTRING, 0x04, 0xDE, 0xAD, 0xBE, 0xEF};
	ret = test_do_action(0x07, 18, &obis_image, 1, block_data, sizeof(block_data), buf, sizeof(buf));
	REQUIRE(ret > 0);
	REQUIRE(buf[0] == 0xC7);
	REQUIRE(buf[3] == 0x00);

	ret = test_do_action(0x08, 18, &obis_image, 5, verify_data, sizeof(verify_data), buf, sizeof(buf));
	REQUIRE(ret > 0);
	REQUIRE(buf[0] == 0xC7);
	REQUIRE(buf[3] == 0x00);
...
TEST_CASE("Integration_SetDataValue", "[integration][basic]") {
	...
	int ret = test_do_set(0x01, 1, &obis_data, 2, set_data, sizeof(set_data), buf, sizeof(buf));
	REQUIRE(ret > 0);
	REQUIRE(buf[0] == 0xC5);
	REQUIRE(buf[3] == 0x00);
...
TEST_CASE("Integration_SetReadOnlyAttributeDenied", "[integration][basic]") {
	...
	uint8_t buf[1024];
	int ret = test_do_set(0x01, 1, &obis_data, 1, set_data, sizeof(set_data), buf, sizeof(buf));
	REQUIRE(ret > 0);
	REQUIRE(buf[0] == 0xC5);
	REQUIRE(buf[3] == 0x03);
}
...
TEST_CASE("Integration_ResetRegister", "[integration][basic]") {
	...
	uint8_t buf[1024];
	int ret = test_do_action(0x01, 3, &obis_register, 1, NULL, 0, buf, sizeof(buf));
	REQUIRE(ret > 0);
	REQUIRE(buf[0] == 0xC7);
	REQUIRE(buf[3] == 0x00);
}
...
TEST_CASE("Integration_DisconnectControlActionsMutateState", "[integration][basic]") {
	...
	ret = test_do_action(0x04, 70, &obis_disconnect, 2, unexpected_payload, sizeof(unexpected_payload), buf, sizeof(buf));
	REQUIRE(ret > 0);
	REQUIRE(buf[0] == 0xC7);
	REQUIRE(buf[3] != 0x00);
...
TEST_CASE("Integration_ProfileGenericCaptureSelectiveAccess", "[integration][service]") {
	...
	ret = test_do_set(0x02, 7, &obis_profile, 3, capture_objects, sizeof(capture_objects), buf, sizeof(buf));
	REQUIRE(ret > 0);
	REQUIRE(buf[0] == 0xC5);
	REQUIRE(buf[3] == 0x00);
...
TEST_CASE("Integration_DataDeviceId", "[integration][service]") {
	...
	int ret = test_do_set(0x01, 1, &obis_data, 2, device_data, sizeof(device_data), set_buf, sizeof(set_buf));
	REQUIRE(ret > 0);
...
TEST_CASE("Integration_ActionNullMethodRejectsUnexpectedPayload", "[integration][service]") {
	...
	ret = test_do_action(0x02, 1, &obis_data, 1, data_value, sizeof(data_value), buf, sizeof(buf));
	REQUIRE(ret > 0);
	REQUIRE(buf[0] == 0xC7);
	REQUIRE(buf[3] == CSM_ACTION_RESULT_OTHER_REASON);
	...
	ret = test_do_action(0x04, 1, &obis_data, 1, null_param, sizeof(null_param), buf, sizeof(buf));
	REQUIRE(ret > 0);
	REQUIRE(buf[0] == 0xC7);
	REQUIRE(buf[3] == 0x00);
...
TEST_CASE("Integration_AssocLnV3AddRemoveObject", "[integration][assoc]") {
	...
	uint8_t add_data[] = {0x02, 0x02, 0x12, 0x00, 0x01, 0x09, 0x06, 0x00, 0x00, 0x60, 0x01, 0x00, 0xFF};
	uint8_t buf[1024];
	int ret = test_do_action(0x01, 15, &obis_asso, 3, add_data, sizeof(add_data), buf, sizeof(buf));
	REQUIRE(ret > 0);
	REQUIRE(buf[0] == 0xC7);
	...
	uint8_t rm_data[] = {0x02, 0x02, 0x12, 0x00, 0x01, 0x09, 0x06, 0x00, 0x00, 0x60, 0x01, 0x00, 0xFF};
	ret = test_do_action(0x03, 15, &obis_asso, 4, rm_data, sizeof(rm_data), buf, sizeof(buf));
	REQUIRE(ret > 0);
	REQUIRE(buf[0] == 0xC7);
}
...
TEST_CASE("Integration_SecuritySetupKeyTransfer", "[integration][security]") {
	...
	uint8_t key_data[] = {0x09, 0x10, 0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0A, 0x0B, 0x0C, 0x0D, 0x0E, 0x0F};
	uint8_t buf[1024];
	int ret = test_do_action(0x01, 64, &obis_security, 2, key_data, sizeof(key_data), buf, sizeof(buf));
	REQUIRE(ret > 0);
	REQUIRE(buf[0] == 0xC7);
	REQUIRE(buf[3] == 0x00);
}
...
TEST_CASE("Integration_SecuritySetupKeyTransferSuite2", "[integration][security]") {
	...
	uint8_t key_data[] = {0x09, 0x20, 0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0A, 0x0B, 0x0C, 0x0D, 0x0E,
	                      0x0F, 0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17, 0x18, 0x19, 0x1A, 0x1B, 0x1C, 0x1D, 0x1E, 0x1F};
	uint8_t buf[1024];
	int ret = test_do_action(0x01, 64, &obis_security, 3, key_data, sizeof(key_data), buf, sizeof(buf));
	REQUIRE(ret > 0);
	REQUIRE(buf[0] == 0xC7);
}
...
TEST_CASE("Integration_SecuritySetupSetSuite2ViaCoSem", "[integration][security]") {
	...
	uint8_t buf[1024];
	int ret = test_do_set(0x01, 64, &obis_security, 3, set_data, sizeof(set_data), buf, sizeof(buf));
	REQUIRE(ret > 0);
	REQUIRE(buf[0] == 0xC5);
...
TEST_CASE("Integration_SecuritySetupSecurityActivate", "[integration][security]") {
	...
	uint8_t buf[1024];
	int ret = test_do_action(0x01, 64, &obis_security, 1, NULL, 0, buf, sizeof(buf));
	REQUIRE(ret > 0);
	REQUIRE(buf[0] == 0xC7);
}
...
TEST_CASE("Integration_SecuritySetupUnsupportedKeyGenerationFails", "[integration][security]") {
	...
	uint8_t buf[1024];
	int ret = test_do_action(0x01, 64, &obis_security, 7, NULL, 0, buf, sizeof(buf));
	REQUIRE(ret > 0);
	REQUIRE(buf[0] == 0xC7);
	REQUIRE(buf[3] == 0x04);
}
...
TEST_CASE("Integration_HlsPendingRejectsAction", "[integration][security]") {
	...
	int ret = test_do_action(0x01, 15, &obis_asso, 1, hls_reply, sizeof(hls_reply), buf, sizeof(buf));
	REQUIRE(ret > 0);
	REQUIRE(buf[0] == AXDR_EXCEPTION_RESPONSE);
	REQUIRE(global_handler_calls == calls_before);
}
