...
static void test_close_socket(test_socket_t fd) {
	closesocket(fd);
}
...
TEST_CASE("Wrapper: reject zero-length APDUs", "[transport][wrapper]") {
	...
	REQUIRE(csm_wrapper_frame_command(apdu, 0U, out, sizeof(out)) == CSM_TRANSPORT_ERR);
	REQUIRE(csm_wrapper_frame_response(apdu, 0U, out, sizeof(out)) == CSM_TRANSPORT_ERR);
	REQUIRE(csm_wrapper_deframe(frame, sizeof(frame), &out_apdu, &out_len) == CSM_TRANSPORT_ERR_FRAMING);
}
...
TEST_CASE("TCP wrapper: frame + deframe roundtrip", "[transport][wrapper]") {
	...
	int flen = csm_tcp_wrapper_frame(0x0001U, 0x0010U, apdu, sizeof(apdu), framed, sizeof(framed));
	...
	int rc = csm_tcp_wrapper_deframe(framed, (uint32_t)flen, &out_apdu, &out_len, &source_wport, &dest_wport);
	REQUIRE(rc == CSM_TRANSPORT_OK);
	REQUIRE(source_wport == 0x0001U);
	REQUIRE(dest_wport == 0x0010U);
	REQUIRE(out_len == sizeof(apdu));
	REQUIRE(memcmp(out_apdu, apdu, sizeof(apdu)) == 0);
	REQUIRE(csm_tcp_wrapper_frame(0x0001U, 0x0010U, apdu, 0U, framed, sizeof(framed)) == CSM_TRANSPORT_ERR_OVERFLOW);
}
...
TEST_CASE("TCP wrapper: reject malformed frames", "[transport][wrapper]") {
	...
	framed[1] = 0x02U;
	REQUIRE(csm_tcp_wrapper_deframe(framed, (uint32_t)flen, &out_apdu, &out_len, NULL, NULL) == CSM_TRANSPORT_ERR_FRAMING);
	...
	framed[7] = 0x10U;
	REQUIRE(csm_tcp_wrapper_deframe(framed, (uint32_t)flen, &out_apdu, &out_len, NULL, NULL) == CSM_TRANSPORT_ERR_TIMEOUT);
}
...
TEST_CASE("TCP transport: receive split wrapper frame", "[transport][tcp]") {
	...
	REQUIRE(csm_transport_tcp_client_init(&transport, "127.0.0.1", port, CSM_FRAMING_TCP_WRAPPER) == CSM_TRANSPORT_OK);
...
TEST_CASE("TCP transport: server accept respects receive timeout", "[transport][tcp]") {
	...
	REQUIRE(csm_transport_tcp_server_init(&transport, 0U, CSM_FRAMING_TCP_WRAPPER) == CSM_TRANSPORT_OK);
	REQUIRE(CSM_TRANSPORT_OPEN(&transport, 0U) == CSM_TRANSPORT_OK);

	uint8_t out[16];
	REQUIRE(CSM_TRANSPORT_RECV(&transport, 0U, out, sizeof(out), 20U) == CSM_TRANSPORT_ERR_TIMEOUT);
...
TEST_CASE("TCP transport: multiple clients keep independent contexts", "[transport][tcp]") {
	...
	REQUIRE(csm_transport_tcp_client_init(&client_a, "127.0.0.1", port_a, CSM_FRAMING_TCP_WRAPPER) == CSM_TRANSPORT_OK);
	REQUIRE(csm_transport_tcp_client_init(&client_b, "127.0.0.1", port_b, CSM_FRAMING_TCP_WRAPPER) == CSM_TRANSPORT_OK);
...
TEST_CASE("TCP transport: client init rejects invalid hosts", "[transport][tcp]") {
	...
	REQUIRE(csm_transport_tcp_client_init(&transport, nullptr, 4060U, CSM_FRAMING_TCP_WRAPPER) == CSM_TRANSPORT_ERR);
	REQUIRE(transport.ops == nullptr);
	REQUIRE(transport.ctx == nullptr);

	REQUIRE(csm_transport_tcp_client_init(&transport, "", 4060U, CSM_FRAMING_TCP_WRAPPER) == CSM_TRANSPORT_ERR);
	REQUIRE(transport.ops == nullptr);
	REQUIRE(transport.ctx == nullptr);
}
...
TEST_CASE("TCP transport: init rejects invalid framing", "[transport][tcp]") {
	...
	REQUIRE(csm_transport_tcp_client_init(&transport, "127.0.0.1", 4060U, invalid) == CSM_TRANSPORT_ERR);
	REQUIRE(transport.ops == nullptr);
	REQUIRE(transport.ctx == nullptr);

	REQUIRE(csm_transport_tcp_server_init(&transport, 4060U, invalid) == CSM_TRANSPORT_ERR);
	REQUIRE(transport.ops == nullptr);
	REQUIRE(transport.ctx == nullptr);
}
...
TEST_CASE("TCP transport: receive rejects null and empty buffers", "[transport][tcp]") {
	...
	REQUIRE(CSM_TRANSPORT_RECV(&transport, 0U, nullptr, sizeof(out), 1000U) == CSM_TRANSPORT_ERR);
	REQUIRE(CSM_TRANSPORT_RECV(&transport, 0U, out, 0U, 1000U) == CSM_TRANSPORT_ERR);
...
TEST_CASE("TCP transport: send rejects null and empty payloads", "[transport][tcp]") {
	...
	REQUIRE(CSM_TRANSPORT_SEND(&transport, 0U, nullptr, sizeof(apdu)) == CSM_TRANSPORT_ERR);
	REQUIRE(CSM_TRANSPORT_SEND(&transport, 0U, apdu, 0U) == CSM_TRANSPORT_ERR);
...
TEST_CASE("Framing: raw passthrough rejects null inputs", "[transport][framing]") {
	...
	REQUIRE(csm_framing_frame(CSM_FRAMING_NONE, 0, nullptr, sizeof(apdu), out, sizeof(out)) == CSM_TRANSPORT_ERR);
	REQUIRE(csm_framing_frame(CSM_FRAMING_NONE, 0, apdu, sizeof(apdu), nullptr, sizeof(out)) == CSM_TRANSPORT_ERR);
}
...
TEST_CASE("Framing: raw deframe rejects null inputs", "[transport][framing]") {
	...
	REQUIRE(csm_framing_deframe(CSM_FRAMING_NONE, nullptr, sizeof(data), &apdu, &apdu_len) == CSM_TRANSPORT_ERR);
	REQUIRE(csm_framing_deframe(CSM_FRAMING_NONE, data, sizeof(data), nullptr, &apdu_len) == CSM_TRANSPORT_ERR);
	REQUIRE(csm_framing_deframe(CSM_FRAMING_NONE, data, sizeof(data), &apdu, nullptr) == CSM_TRANSPORT_ERR);
}
...
static int timeout_transport_recv(void *ctx, uint8_t channel, uint8_t *buf, uint32_t buf_size, uint32_t timeout_ms) {
	timeout_transport_ctx_t *tctx = (timeout_transport_ctx_t *)ctx;
	(void)channel;
	(void)buf;
	(void)buf_size;
...
static int capture_transport_recv(void *ctx, uint8_t channel, uint8_t *buf, uint32_t buf_size, uint32_t timeout_ms) {
	capture_transport_ctx_t *tctx = (capture_transport_ctx_t *)ctx;
	(void)channel;
	(void)timeout_ms;
	REQUIRE(buf_size >= 9U);
...
TEST_CASE("Client: request APIs reject invalid input before transport send", "[transport][client]") {
	static const csm_transport_ops timeout_ops = {timeout_transport_open, timeout_transport_send, timeout_transport_recv, NULL, NULL, NULL};
	...
	REQUIRE(csm_client_get(client, 1U, 3U, nullptr, 2U, resp, sizeof(resp)) == -1);
	REQUIRE(csm_client_get(client, 1U, 3U, &obis, 2U, nullptr, sizeof(resp)) == -1);
	REQUIRE(csm_client_get(client, 1U, 3U, &obis, 2U, resp, 0U) == -1);
	REQUIRE(csm_client_set(client, 1U, 3U, nullptr, 2U, data, sizeof(data), resp, sizeof(resp)) == -1);
	REQUIRE(csm_client_set(client, 1U, 3U, &obis, 2U, nullptr, 1U, resp, sizeof(resp)) == -1);
	REQUIRE(csm_client_action(client, 1U, 70U, nullptr, 1U, data, sizeof(data), resp, sizeof(resp)) == -1);
	REQUIRE(csm_client_action(client, 1U, 70U, &obis, 1U, nullptr, 1U, resp, sizeof(resp)) == -1);
	REQUIRE(csm_client_get_block(client, 1U, 3U, nullptr, 2U, resp, sizeof(resp)) == -1);
	REQUIRE(csm_client_set_block(client, 1U, 3U, &obis, 2U, nullptr, 1U, resp, sizeof(resp)) == -1);
...
TEST_CASE("Client: set block encodes long octet-string chunk lengths", "[transport][client]") {
	static const csm_transport_ops capture_ops = {capture_transport_open, capture_transport_send, capture_transport_recv, NULL, NULL, NULL};
	...
	REQUIRE(client != nullptr);
	REQUIRE(csm_client_set_block(client, 1U, 3U, &obis, 2U, data, sizeof(data), resp, sizeof(resp)) == 4);
...
TEST_CASE("Framing: HDLC dispatch rejects unsafe inputs", "[transport][framing][hdlc]") {
	...
	REQUIRE(csm_framing_frame(CSM_FRAMING_HDLC, 0, nullptr, sizeof(apdu), out, sizeof(out)) == CSM_TRANSPORT_ERR);
	REQUIRE(csm_framing_frame(CSM_FRAMING_HDLC, 0, apdu, sizeof(apdu), nullptr, sizeof(out)) == CSM_TRANSPORT_ERR);
	REQUIRE(csm_framing_frame(CSM_FRAMING_HDLC, 0, apdu, sizeof(apdu), out, sizeof(out)) == CSM_TRANSPORT_ERR_OVERFLOW);

	REQUIRE(csm_framing_deframe(CSM_FRAMING_HDLC, nullptr, sizeof(frame), &out_apdu, &out_len) == CSM_TRANSPORT_ERR);
	REQUIRE(csm_framing_deframe(CSM_FRAMING_HDLC, frame, sizeof(frame), nullptr, &out_len) == CSM_TRANSPORT_ERR);
	REQUIRE(csm_framing_deframe(CSM_FRAMING_HDLC, frame, sizeof(frame), &out_apdu, nullptr) == CSM_TRANSPORT_ERR);
}
