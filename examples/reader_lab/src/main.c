...
static int parse_decimal_int(const char *str, int min_value, int max_value, int *out) {
	...
	if ((str == NULL) || (out == NULL) || (min_value > max_value) || (*str < '0') || (*str > '9')) {
		return -1;
	}
...
static void setup_profile(const char *profile, opendlms_reader_auth_t *auth, csm_asso_config *asso_cfg, uint8_t plain_hls) {
	...
	if (strcmp(profile, "reader") == 0) {
		...
		static const uint8_t lab_client_title[8] = {0x41U, 0x42U, 0x43U, 0x44U, 0x45U, 0x46U, 0x47U, 0x48U};
		...
		reader_hal_keyring_set_hex(48U, "000102030405060708090A0B0C0D0E0F", "D0D1D2D3D4D5D6D7D8D9DADBDCDDDEDF", "FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF");
...
static int is_known_profile(const char *profile) {
	return (strcmp(profile, "public") == 0) || (strcmp(profile, "reader") == 0) || (strcmp(profile, "config") == 0) || (strcmp(profile, "configurator") == 0);
}
...
int main(int argc, char **argv) {
	...
	if (set_hex != NULL) {
		if (parse_hex(set_hex, set_hex_buf, sizeof(set_hex_buf), &set_hex_len) < 0 || set_hex_len == 0U) {
	...
	opendlms_reader_session_set_transport(&session, OPENDLMS_READER_TRANSPORT_TCP_WRAPPER, asso_cfg.llc.ssap, dest_wport);
	...
	if (set_hex != NULL) {
		printf(
		    "SET %u.%u.%u.%u.%u.%u class=%u attr=%d hex_len=%lu\n", obis.A, obis.B, obis.C, obis.D, obis.E, obis.F, (unsigned)class_id, attr_override,
		    (unsigned long)set_hex_len
		);

		if (opendlms_reader_set(&session, class_id, &obis, (uint8_t)attr_override, set_hex_buf, set_hex_len, &response) < 0) {
			fprintf(stderr, "SET failed\n");
			operation_failed = 1;
		} else {
			printf("SET OK: service=%d result=%u access=%u\n", (int)response.service, (unsigned)response.result, (unsigned)response.access_result);
		}
	} else if (set_u32 >= 0) {
		uint8_t set_data[] = {
		    AXDR_TAG_UNSIGNED32, (uint8_t)(((uint32_t)set_u32 >> 24U) & 0xFFU), (uint8_t)(((uint32_t)set_u32 >> 16U) & 0xFFU),
		    (uint8_t)(((uint32_t)set_u32 >> 8U) & 0xFFU), (uint8_t)((uint32_t)set_u32 & 0xFFU)
		};

		printf("SET %u.%u.%u.%u.%u.%u class=%u attr=%d value=%d\n", obis.A, obis.B, obis.C, obis.D, obis.E, obis.F, (unsigned)class_id, attr_override, set_u32);

		if (opendlms_reader_set(&session, class_id, &obis, (uint8_t)attr_override, set_data, sizeof(set_data), &response) < 0) {
			fprintf(stderr, "SET failed\n");
			operation_failed = 1;
		} else {
			printf("SET OK: service=%d result=%u access=%u\n", (int)response.service, (unsigned)response.result, (unsigned)response.access_result);
		...
		printf("ACTION %u.%u.%u.%u.%u.%u class=%u method=%d\n", obis.A, obis.B, obis.C, obis.D, obis.E, obis.F, (unsigned)class_id, action_method);

		if (opendlms_reader_action(&session, class_id, &obis, (uint8_t)action_method, NULL, 0U, &response) < 0) {
		...
		if (opendlms_reader_get(&session, class_id, &obis, (uint8_t)attr_override, &response) < 0) {
