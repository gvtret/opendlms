...
int csm_server_init(csm_server *server, csm_transport *transport, uint8_t channel, csm_framing_type framing) {
	...
	csm_channel_ctx_init(&server->chan_ctx, server->channels, CSM_SERVER_MAX_CHANNELS, server->asso_states, server->asso_configs, CSM_SERVER_MAX_CHANNELS);
...
int csm_server_poll(csm_server *server, uint32_t timeout_ms) {
	...
	int recv_len = CSM_TRANSPORT_RECV(server->transport, ch, server->rx_buf, sizeof(server->rx_buf), timeout_ms);

	if (recv_len <= 0)
		return recv_len; /* Timeout or error */

	/* Deframe if needed */
	const uint8_t *apdu;
	uint32_t apdu_len;
	int rc = csm_framing_deframe(server->framing, server->rx_buf, recv_len, &apdu, &apdu_len);
	if (rc != 0)
		return -1;
	...
	if (apdu_len > sizeof(server->tx_buf))
		return -1;
	...
	if (resp_len > 0) {
		...
		int framed_len = csm_framing_frame(server->framing, 1, server->tx_buf, resp_len, framed, sizeof(framed));
...
int csm_server_send(csm_server *server, uint8_t channel, const uint8_t *apdu, uint32_t apdu_len) {
	if (!server || !server->transport || !apdu || apdu_len == 0U)
		return -1;

	uint8_t framed[CSM_WRAPPER_MAX_LEN];
	int framed_len = csm_framing_frame(server->framing, 1, apdu, apdu_len, framed, sizeof(framed));
	if (framed_len < 0)
		return framed_len;
...
static int client_send_recv(csm_client *client, uint8_t *apdu, uint32_t apdu_len, uint8_t *resp_buf, uint32_t resp_size);
...
int csm_dlms_client_init(csm_client *client, csm_transport *transport, uint8_t channel, csm_framing_type framing) {
	...
	csm_channel_ctx_init(&client->chan_ctx, client->channels, CSM_SERVER_MAX_CHANNELS, client->asso_states, client->asso_configs, CSM_SERVER_MAX_CHANNELS);
...
static int client_hls_gmac_handshake(csm_client *client, csm_asso_state *asso) {
	...
	uint8_t resp[64];
	int rlen = csm_client_action(client, 1U, 15U, &current_asso, 1U, param, sizeof(param), resp, sizeof(resp));
	if (rlen <= 0) {
		return -1;
	}
	...
	if ((rlen < 25) || (resp[0] != (uint8_t)AXDR_ACTION_RESPONSE) || (resp[3] != 0x00U) || (resp[6] != (uint8_t)AXDR_TAG_OCTETSTRING) || (resp[7] != 17U)) {
		return -1;
	}
...
int csm_client_connect(csm_client *client, uint32_t timeout_ms) {
	...
	asso->ref = (client->asso_configs[0].application_context != 0U) ? (enum csm_referencing)client->asso_configs[0].application_context : LN_REF;
	...
	if (!csm_asso_encoder(asso, &req, CSM_ASSO_AARQ))
		return -1;

	int resp_len = client_send_recv(client, client->tx_buf, req.wr_index, client->rx_buf, sizeof(client->rx_buf));
	if (resp_len <= 0)
		return resp_len;

	csm_array resp;
	csm_array_init(&resp, client->rx_buf, sizeof(client->rx_buf), (uint32_t)resp_len, 0);
	if (!csm_asso_decoder(asso, &resp, CSM_ASSO_AARE))
		return -1;
	if (!asso->handshake.accepted)
		return -1;
...
static int client_send_recv(csm_client *client, uint8_t *apdu, uint32_t apdu_len, uint8_t *resp_buf, uint32_t resp_size) {
	if (!client || !client->transport || !apdu || apdu_len == 0U || !resp_buf || resp_size == 0U) {
		return -1;
	}
	...
	/* Frame and send */
	uint8_t framed[CSM_WRAPPER_MAX_LEN];
	int framed_len = csm_framing_frame(client->framing, 0, apdu, apdu_len, framed, sizeof(framed));
	if (framed_len < 0)
		return framed_len;
	...
	int recv_len = CSM_TRANSPORT_RECV(client->transport, ch, rx_framed, sizeof(rx_framed), client->rx_timeout_ms);
	if (recv_len <= 0)
		return recv_len;
	...
	int rc = csm_framing_deframe(client->framing, rx_framed, recv_len, &resp_apdu, &resp_apdu_len);
	if (rc != 0)
		return -1;

	if (resp_apdu_len > resp_size)
		return -1;
...
int csm_client_get(
	...
	return client_send_recv(client, client->tx_buf, req.wr_index, resp_buf, resp_size);
}
...
int csm_client_set(
	...
	if (!client || !obis || !resp_buf || resp_size == 0U || (!data && data_len > 0U)) {
		return -1;
	}
 ...
 *  -1 = error
 */
static int client_decode_response_type(const uint8_t *resp, uint32_t resp_len, uint8_t *invoke_id, uint8_t *last_block, uint32_t *block_number) {
	if (resp_len < 8U)
		return -1;
	...
	if ((tag == 0xC4U) || (tag == 0xC5U) || (tag == 0xC7U)) {
		if (type == 0x04U) /* With DataBlock */
		{
			*invoke_id = resp[2];
			*last_block = resp[3];
			*block_number = ((uint32_t)resp[4] << 24) | ((uint32_t)resp[5] << 16) | ((uint32_t)resp[6] << 8) | (uint32_t)resp[7];
...
int csm_client_get_block(
	...
	int resp_len = csm_client_get(client, invoke_id, class_id, obis, attr_id, resp_buf, resp_size);
	if (resp_len <= 0)
		return resp_len;
	...
	int type = client_decode_response_type(resp_buf, (uint32_t)resp_len, &resp_invoke_id, &last_block, &block_number);
	...
	while (!last_block) {
		...
		int rx_len = client_send_recv(client, client->tx_buf, req.wr_index, rx_buf, sizeof(rx_buf));
		if (rx_len <= 0)
			return rx_len;

		/* Decode block response */
		type = client_decode_response_type(rx_buf, (uint32_t)rx_len, &resp_invoke_id, &last_block, &block_number);
		if (type != 1)
			return -1; /* Expected block response */

		/* Copy data from this block (skip 8-byte header) */
		if ((uint32_t)rx_len > 8U) {
			uint32_t data_len = (uint32_t)rx_len - 8U;
			if ((total_offset + data_len) > resp_size)
				return -1;
...
int csm_client_set_block(
	...
	if ((header_overhead + data_len) <= CSM_CLIENT_MAX_BLOCK_SIZE) {
		...
		return csm_client_set(client, invoke_id, class_id, obis, attr_id, data, data_len, resp_buf, resp_size);
	}
