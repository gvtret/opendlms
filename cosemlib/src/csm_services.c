...
static csm_db_code
svc_get_request_decoder(csm_db_access_handler handler, csm_db_context_t *ctx, csm_asso_state *state, csm_request *request, csm_array *array) {
	...
	if (svc_decode_request(request, array)) {
		...
		if (request->type == SVC_REQUEST_NEXT) {
			if (csm_block_is_active(&state->block_transfer) && (state->block_transfer.invoke_id == request->sender_invoke_id)) {
				CSM_LOG("[SVC] GET-Request-Next, block %lu", (unsigned long)state->block_transfer.block_number);
		...
		if (handler != NULL) {
			...
			if (code == CSM_OK_BLOCK) {
				...
				if ((code == CSM_OK) || (code == CSM_OK_BLOCK)) {
					...
					if (csm_block_start_server(&state->block_transfer, request->sender_invoke_id, temp.buff, temp.wr_index, 0U)) {
						/* Encode first block */
						array->wr_index = 0U;
...
static csm_db_code
svc_set_or_action_execute(csm_db_access_handler handler, csm_db_context_t *ctx, csm_asso_state *state, csm_request *request, csm_array *array) {
	csm_db_code code = CSM_ERR_BAD_ENCODING;
	(void)state;
	...
	if (request->type == SVC_REQUEST_NORMAL) {
		if (handler != NULL) {
			...
			csm_array *input = (request->db_request.additional_data.enable != 0U) ? &request->db_request.additional_data.data : array;
...
static csm_db_code
svc_set_or_action_decoder(csm_db_access_handler handler, csm_db_context_t *ctx, csm_asso_state *state, csm_request *request, csm_array *array) {
	if (!svc_decode_request(request, array)) {
...
static csm_db_code
svc_set_request_decoder(csm_db_access_handler handler, csm_db_context_t *ctx, csm_asso_state *state, csm_request *request, csm_array *array) {
	csm_db_code code = CSM_ERR_BAD_ENCODING;
	...
	if (svc_decode_request(request, array)) {
		...
		if (request->type == SVC_REQUEST_NEXT) {
			if (csm_block_is_active(&state->block_transfer) && (state->block_transfer.invoke_id == request->sender_invoke_id)) {
				CSM_LOG("[SVC] SET-Request-Next, block %lu", (unsigned long)state->block_transfer.block_number);
...
static csm_db_code
svc_action_request_decoder(csm_db_access_handler handler, csm_db_context_t *ctx, csm_asso_state *state, csm_request *request, csm_array *array) {
...
int svc_request_encoder(csm_request *request, csm_array *array) {
	...
	uint8_t tag = (request->db_request.service == SVC_GET) ? AXDR_GET_REQUEST
	    : (request->db_request.service == SVC_SET)         ? AXDR_SET_REQUEST
	                                                       : AXDR_ACTION_REQUEST;
		...
		if (tag != AXDR_ACTION_REQUEST) {
			...
			if (request->db_request.sel_access.enable) {
				if (request->db_request.sel_access.data.buff != NULL) {
					valid =
					    valid && csm_array_write_buff(array, request->db_request.sel_access.data.buff, csm_array_written(&request->db_request.sel_access.data));
				} else {
					valid = FALSE;
				}
			}
		...
		if (tag != AXDR_GET_REQUEST) {
			...
			if ((request->db_request.additional_data.enable) && (data_size > 0U)) {
				if (request->db_request.additional_data.data.buff != NULL) {
					valid = valid &&
					    csm_array_write_buff(
					            array, request->db_request.additional_data.data.buff, csm_array_written(&request->db_request.additional_data.data)
					    );
				} else {
					valid = FALSE;
				}
...
int svc_result_decoder(csm_response *response, csm_array *array) {
	...
	} else if (response->service == SVC_GET) {
	...
	if (valid && (response->type == SVC_RESPONSE_WITH_DATABLOCK)) {
		...
		if (data_size > 0U) {
			...
			if (!csm_block_is_active(&response->block_state)) {
				csm_block_start_get_receive(&response->block_state, response->invoke_id, 0U);
			}

			/* Accumulate this block's data */
			const uint8_t *data_ptr = csm_array_rd_data(array);
			if (csm_block_get_receive_data(&response->block_state, data_ptr, data_size, response->last_block)) {
				CSM_LOG(
				    "[SVC/GET] Accumulated %lu bytes, block %lu, last=%d", (unsigned long)data_size, (unsigned long)response->block_number,
...
int csm_client_encode_selective_access_by_range(csm_array *array, csm_object_t *restricting_object, csm_array *start, csm_array *end) {
	...
	if (((start_size > 0U) && (start_data == NULL)) || ((end_size > 0U) && (end_data == NULL))) {
		return FALSE;
	}
...
void csm_client_init(csm_request *request, csm_response *response) {
	...
	response->access_result = CSM_ACCESS_RESULT_NOT_SET;
	response->action_result = CSM_ACTION_RESULT_NOT_SET;
	csm_block_init(&response->block_state);
}

int csm_client_has_more_data(csm_response *response) {
	int more_data = 0;
	if ((response != NULL) && (response->type == SVC_RESPONSE_WITH_DATABLOCK) && (!response->last_block)) {
		more_data = 1;
	}
	return more_data;
}

int csm_client_get_block_data(csm_response *response, const uint8_t **data, uint32_t *data_size) {
