...
static csm_acse_code acse_auth_value_decoder(csm_asso_state *state, csm_ber *ber, csm_array *array, uint8_t tag) {
	...
	if (csm_ber_decode(ber, array)) {
		...
		if ((ber->length.length >= (CSM_DEF_LLS_MIN_SIZE)) && (ber->length.length <= (CSM_DEF_CHALLENGE_SIZE))) {
					...
					if (csm_array_read_u8(array, &byte)) {
						if (byte != AXDR_TAG_NULL) {
							...
							if (byte == AXDR_TAG_OCTETSTRING) {
								uint8_t key_len = 0;
								if (csm_array_read_u8(array, &key_len) && (key_len <= sizeof(state->dedicated_key)) && (key_len <= csm_array_unread(array))) {
...
int csm_asso_is_granted(csm_asso_state *state) {
	...
	if (state->state_cf == CF_IDLE) {
		...
		if (state->auth_level == CSM_AUTH_LOWEST_LEVEL) {
			...
			csm_hal_get_lls_password((uint8_t)state->config->llc.dsap, &state->handshake.stoc.value[0], 8U);
		...
		if (csm_array_write_u8(array, 0U)) {
			...
			for (i = 0U; i < size; i++) {
				...
				if ((codec[i].insert_func != NULL) && (codec[i].context != ACSE_SKIP)) {
					...
					else if ((codec[i].context == ACSE_SEC_TITLE) && !asso_auth_uses_system_title(state->auth_level)) {
						continue;
					} else {
			...
			if (i >= size) {
				...
				if (payload_len < 128U) {
					...
					memmove(&array->buff[array->offset + 3U], &array->buff[array->offset + 2U], payload_len);
					array->wr_index++;
					...
					memmove(&array->buff[array->offset + 4U], &array->buff[array->offset + 2U], payload_len);
					array->wr_index += 2U;
	...
	if (asso->state_cf == CF_IDLE) {
		if (csm_asso_decoder(asso, packet, CSM_ASSO_AARQ)) {
			if (csm_asso_is_granted(asso)) {
				...
				static const char *reason_text[] = {"accepted", "rejected-permanent", "rejected-transient", "rejected-authentication", "rejected-context"};
				const char *reason = (asso->handshake.result <= 4U) ? reason_text[asso->handshake.result] : "unknown";
