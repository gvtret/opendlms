...
int csm_block_encode_first(csm_block_state *state, csm_array *array, uint32_t max_size) {
	if ((state == NULL) || (array == NULL) || (!state->active) || (max_size == 0U) || (state->offset > state->total_size)) {
		return 0;
	}
...
int csm_block_encode_set_request(csm_block_state *state, csm_array *array, const csm_request *request, uint32_t max_size) {
	if ((state == NULL) || (array == NULL) || (request == NULL) || (!state->active) || (max_size == 0U) || (state->offset > state->total_size)) {
		return 0;
	}
...
int csm_block_receive_data(csm_block_state *state, const uint8_t *data, uint32_t data_size, uint8_t is_last) {
	...
	if ((state->offset > CSM_BLOCK_MAX_RECEIVE_SIZE) || (data_size > (CSM_BLOCK_MAX_RECEIVE_SIZE - state->offset))) {
