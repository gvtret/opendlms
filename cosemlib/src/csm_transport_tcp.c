...
static void tcp_close_channel(csm_tcp_context *ctx, uint8_t channel) {
	if (channel < CSM_TCP_MAX_CLIENTS && ctx->channels[channel].connected) {
		...
		if (ctx->event_cb) {
			ctx->event_cb(channel, CSM_TRANSPORT_EVT_DISCONNECTED, NULL, 0, ctx->event_ctx);
		}
	}
}
...
static int tcp_extract_pdu(csm_tcp_context *ctx, uint8_t channel, uint8_t *out, uint32_t out_size, uint32_t *consumed) {
	csm_tcp_channel *ch = &ctx->channels[channel];
	if (!out || out_size == 0U || !consumed)
	...
	if (ctx->framing == CSM_FRAMING_WRAPPER) {
		...
		*consumed = total_len;
		...
		const uint8_t *frame;
		uint32_t frame_len;
		int rc = csm_hdlc_find_frame(ch->recv_buf, ch->recv_len, &frame, &frame_len, consumed);
		if (rc != CSM_TRANSPORT_OK)
			return 0;
...
static int tcp_send(void *ctx, uint8_t channel, const uint8_t *data, uint32_t len) {
	...
	if (!data || len == 0U)
		return CSM_TRANSPORT_ERR;
	if (!c->channels[channel].connected)
		return CSM_TRANSPORT_ERR_CONN;
	...
	while (sent < framed_len) {
		int n = (int)send(c->channels[channel].fd, (const char *)(framed + sent), framed_len - sent, 0);
		if (n <= 0)
			return CSM_TRANSPORT_ERR_IO;
		sent += n;
	}
...
static int tcp_recv(void *ctx, uint8_t channel, uint8_t *buf, uint32_t buf_size, uint32_t timeout_ms) {
	csm_tcp_context *c = (csm_tcp_context *)ctx;
	if (channel >= CSM_TCP_MAX_CLIENTS)
		return CSM_TRANSPORT_ERR;
	if (!buf || buf_size == 0U)
	...
	if (c->is_server && !c->channels[channel].connected) {
		...
		if (c->event_cb) {
			c->event_cb(channel, CSM_TRANSPORT_EVT_CONNECTED, NULL, 0, c->event_ctx);
		}
	}
	...
	for (;;) {
		...
		if (pdu_len > 0 && consumed > 0) {
			...
			memmove(c->channels[channel].recv_buf, c->channels[channel].recv_buf + consumed, c->channels[channel].recv_len - consumed);
...
static const csm_transport_ops tcp_ops = {
    .open = tcp_open, .send = tcp_send, .recv = tcp_recv, .close = tcp_close, .is_connected = tcp_is_connected, .destroy = tcp_destroy
};
...
static int tcp_is_valid_framing(csm_framing_type framing) {
	return (framing == CSM_FRAMING_NONE) || (framing == CSM_FRAMING_WRAPPER) || (framing == CSM_FRAMING_TCP_WRAPPER) || (framing == CSM_FRAMING_HDLC);
}

int csm_transport_tcp_server_init(csm_transport *transport, uint16_t port, csm_framing_type framing) {
	if (!transport)
		return CSM_TRANSPORT_ERR;
	if (!tcp_is_valid_framing(framing))
		return CSM_TRANSPORT_ERR;
...
int csm_transport_tcp_client_init(csm_transport *transport, const char *host, uint16_t port, csm_framing_type framing) {
	...
	if (host_len >= sizeof(client_ctx->host))
		host_len = sizeof(client_ctx->host) - 1;
