...
int csm_model_instance_add(uint16_t class_id, const csm_obis_code *obis, uint8_t version) {
	...
	for (int i = 0; i < instance_count; i++) {
		if (instance_table[i].class_id == class_id && obis_equal(&instance_table[i].obis, obis)) {
			CSM_ERR("[INSTANCE] Duplicate class=%u OBIS=%02X%02X%02X%02X%02X%02X", class_id, obis->A, obis->B, obis->C, obis->D, obis->E, obis->F);
			return FALSE;
		}
	}
	...
	instance_count++;
	CSM_LOG("[INSTANCE] Added class=%u OBIS=%02X%02X%02X%02X%02X%02X v%u", class_id, obis->A, obis->B, obis->C, obis->D, obis->E, obis->F, version);
	return TRUE;
}
