...
static void scalar_to_ymd(uint32_t scalar, uint32_t *pyr, uint32_t *pmo, uint32_t *pday) {
...
uint32_t clk_to_epoch(struct tm *timeptr) {
	...
	if ((timeptr->tm_year < 70) || (timeptr->tm_year > 120)) {
		...
		tt = ymd_to_scalar(timeptr->tm_year + 1900, timeptr->tm_mon + 1, timeptr->tm_mday) - ymd_to_scalar(1970, 1, 1);
...
char *MoonPhaseText[8] = {"new", "waxing crescent", "first quarter", "waxing gibbous", "full", "waning gibbous", "third quarter", "waning crescent"};
...
int add_time(
    uint32_t basehrs, uint32_t basemins, uint32_t basesecs, int spanhrs, int spanmins, int spansecs, uint32_t *hrs, uint32_t *mins, uint32_t *secs,
    uint32_t *days
) {
	int h, m, s;
	div_t r;
...
void clk_cosem_update_status(clk_datetime_t *clk) {
	...
	if (!clk_is_valid_date(clk->date.year, clk->date.month, clk->date.day) || !clk_is_valid_time(clk->time.hour, clk->time.minute, clk->time.second) ||
	    !clk_is_valid_deviation(clk->deviation) || (isDst < 0)) {
		invalid_bit = 1;
	} else {
		if (isDst) {
			dst_bit = 1U;
