...
static const db_ic_object_descr clock_descr = {
    .attributes = clock_attrs, .methods = clock_methods, .class_id = 8, .obis = {0, 0, 1, 0, 0, 255},
               .attr_count = 9, .method_count = 2, .version = 0
};
...
static const uint8_t clock_default_dt[DB_CLOCK_DT_LEN] = {0x07, 0xD2, 0x0C, 0x04, 0x03, 0x0A, 0x06, 0x0B, 0xFF, 0x00, 0x78, 0x00};
...
static void clock_increment_hour(db_cosem_clock_data_t *clk) {
	...
	clk->hour = 0U;
	clk->day++;
	if (clk->day <= clock_days_in_month(clk->year, clk->month)) {
		return;
	}
...
static csm_db_code clock_dispatch(db_ic_inst_t *inst, db_ic_op_t op, uint8_t attr_id, uint8_t method_id, csm_array *in, csm_array *out) {
	...
	if (op == IC_OP_GET) {
		if (attr_id == 1U) {
			...
			return csm_axdr_wr_octetstring(out, (const uint8_t *)&obis->A, 6U) ? CSM_OK : CSM_ERR_OBJECT_ERROR;
		}

		if (attr_id == 2U) {
			uint8_t dt[DB_CLOCK_DT_LEN];
			db_cosem_clock_get_datetime(dt, inst);
			return csm_axdr_wr_octetstring(out, dt, DB_CLOCK_DT_LEN) ? CSM_OK : CSM_ERR_OBJECT_ERROR;
		}
		...
		if (attr_id == 2U) {
			...
			if (!csm_array_read_u8(in, &tag) || !csm_array_read_u8(in, &len) || (tag != AXDR_TAG_OCTETSTRING) || (len != DB_CLOCK_DT_LEN) ||
		...
		if (method_id == 2U) {
			if ((in != NULL) && (csm_array_unread(in) != 0U)) {
				return CSM_ERR_BAD_ENCODING;
			}
...
static const db_ic_class ic_clock = {.class_id = 8, .name = "Clock", .version = 0, .descr = &clock_descr, .create = clock_create, .dispatch = clock_dispatch};

void db_ic_register_clock(void) {
	db_ic_register(&ic_clock);
}
