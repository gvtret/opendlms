...
static csm_db_code sec_dispatch(db_ic_inst_t *inst, db_ic_op_t op,
                                  ...
                                  csm_array *in, csm_array *out)
{
    ...
    if (op == IC_OP_GET)
    {
        ...
        case 4U:
        {
            ...
            {
                valid = valid && csm_array_write_buff(out, sd->key_transfer_buf,
                                                        sd->key_transfer_len);
            }
            return valid ? CSM_OK : CSM_ERR_BAD_ENCODING;
        }
...
static const db_ic_class ic_sec = {
    .class_id  = 64,
    .name      = "Security Setup",
    .version   = 1,
    .create    = sec_create,
    .dispatch  = sec_dispatch
};
