...
void db_ic_register_activation_reset_count(void) {
	reg_act_pool_count = 0U;
}
...
static const db_ic_object_descr reg_act_descr = {
    .attributes = reg_act_attrs, .methods = reg_act_methods, .class_id = 6, .obis = {0, 0, 60, 5, 0, 255},
               .attr_count = 4, .method_count = 3, .version = 0
};

static db_ic_inst_t *reg_act_create(const csm_obis_code *obis) {
	(void)obis;
	if (reg_act_pool_count >= REG_ACT_MAX_INSTANCES) {
		return NULL;
	}
...
static int reg_act_read_object(csm_array *in, db_ic_reg_act_obj *obj) {
	...
	if (!csm_array_read_u8(in, &obis_len) || obis_len != sizeof(obis)) {
		return FALSE;
	}
	if (!csm_array_read_buff(in, obis, sizeof(obis))) {
		return FALSE;
	}
	obj->obis.A = obis[0];
	obj->obis.B = obis[1];
	...
	if (!csm_array_read_u8(in, &obj->attribute_id)) {
		return FALSE;
	}
	return TRUE;
}
...
static int reg_act_read_mask(csm_array *in, db_ic_reg_act_mask *mask) {
	...
	if (!csm_array_read_u16(in, &mask->mask_id)) {
		return FALSE;
	}
	...
	if (!csm_array_read_u8(in, &entry_count) || entry_count > 4U) {
		return FALSE;
	}
	...
	for (uint8_t i = 0U; i < entry_count; i++) {
		...
		if (!csm_array_read_u8(in, &entry_fields) || entry_fields != 2U) {
			return FALSE;
		}
		...
		if (!csm_array_read_u16(in, &mask->entries[i].mask_id)) {
			return FALSE;
		}
		...
		if (!csm_array_read_u16(in, &mask->entries[i].object_index)) {
			return FALSE;
		}
	}
...
static csm_db_code reg_act_dispatch(db_ic_inst_t *inst, db_ic_op_t op, uint8_t attr_id, uint8_t method_id, csm_array *in, csm_array *out) {
	...
	if (op == IC_OP_GET) {
		...
		switch (method_id) {
			case 1U:
				if (d->object_count >= REG_ACT_MAX_OBJECTS) {
					return CSM_ERR_DATA_CONTENT_NOT_OK;
				}
				if (!reg_act_read_object(in, &d->objects[d->object_count])) {
			...
			case 2U:
				if (d->mask_count >= REG_ACT_MAX_MASKS) {
					return CSM_ERR_DATA_CONTENT_NOT_OK;
				}
				if (!reg_act_read_mask(in, &d->masks[d->mask_count])) {
					return CSM_ERR_BAD_ENCODING;
				}
				d->mask_count++;
				return CSM_OK;
			...
			case 3U: /* delete_mask: read Unsigned16, remove from mask list */
			{
				...
				uint16_t mask_id = 0U;
				if (!csm_array_read_u16(in, &mask_id)) {
					return CSM_ERR_BAD_ENCODING;
				}
...
static const db_ic_class ic_reg_act = {.class_id = 6, .name = "Register Activation", .version = 0, .create = reg_act_create, .dispatch = reg_act_dispatch};
