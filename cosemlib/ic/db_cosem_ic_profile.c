...
void db_ic_profile_reset_count(void) {
	pg_pool_count = 0U;
	pg_inst_count = 0U;
}
...
static const db_ic_object_descr pg_descr = {
    .attributes = pg_attrs, .methods = pg_methods, .class_id = 7, .obis = {0, 0, 0, 0, 0, 0},
               .attr_count = 8, .method_count = 4, .version = 1
};
...
static int pg_encode_capture_obj(csm_array *out, const db_ic_profile_capture_obj *obj) {
...
static csm_db_code pg_get_buffer(const db_ic_profile_data *pg, csm_array *out) {
	...
	for (uint16_t i = 0U; i < pg->entries_in_use && valid; i++) {
		...
		for (uint8_t j = 0U; j < entry->value_count && valid; j++) {
			valid = valid && csm_array_write_buff(out, entry->values[j], entry->value_len[j]);
		}
	}
...
static csm_db_code pg_get_capture_objects(const db_ic_profile_data *pg, csm_array *out) {
	int valid = csm_array_write_u8(out, AXDR_TAG_ARRAY);
...
static csm_db_code pg_read_entries_by_range(db_ic_profile_data *pg, csm_array *in, csm_array *out) {
	uint8_t tag = 0xFFU;
	if (!csm_array_read_u8(in, &tag) || tag != AXDR_TAG_STRUCTURE) {
		return CSM_ERR_BAD_ENCODING;
	}
...
static csm_db_code pg_read_entries_by_index(db_ic_profile_data *pg, csm_array *in, csm_array *out) {
	uint8_t tag = 0xFFU;
	if (!csm_array_read_u8(in, &tag) || tag != AXDR_TAG_STRUCTURE) {
		return CSM_ERR_BAD_ENCODING;
	}
...
static csm_db_code pg_capture(db_ic_profile_data *pg) {
	...
	for (uint8_t i = 0U; i < pg->capture_count; i++) {
		...
		csm_db_code code = (csm_db_code)db_ic_dispatch(captured, IC_OP_GET, pg->captures[i].attribute_id, 0U, &capture_in, &capture_out);
		if (code != CSM_OK) {
			return code;
		}
...
static csm_db_code pg_dispatch(db_ic_inst_t *inst, db_ic_op_t op, uint8_t attr_id, uint8_t method_id, csm_array *in, csm_array *out) {
	(void)method_id;
...
static const db_ic_class ic_pg = {.class_id = 7, .name = "Profile Generic", .version = 1, .create = pg_create, .dispatch = pg_dispatch};
