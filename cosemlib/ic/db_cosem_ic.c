                      ...
                      const void *init_data, void *user_ctx)
{
    ...
    instance_count++;
    CSM_LOG("[IC] Created instance class=%u, OBIS=%02X%02X%02X%02X%02X%02X",
               ...
               db_ic_inst_t **out)
{
    ...
    for (uint8_t i = 0U; i < instance_count; i++)
    {
        ...
        const csm_obis_code *inst_obis = instances[i].has_obis
            ? &instances[i].obis
            : &instances[i].descr->obis;

        if ((instances[i].descr->class_id == class_id) &&
            obis_equal(inst_obis, obis))
        {
            *out = &instances[i];
            return TRUE;
        }
