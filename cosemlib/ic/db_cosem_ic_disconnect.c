...
{
    (void) obis;
    if (disc_pool_count >= DISC_MAX_INSTANCES) { return NULL; }
...
static csm_db_code disc_dispatch(db_ic_inst_t *inst, db_ic_op_t op,
                                   ...
                                   csm_array *in, csm_array *out)
{
    ...
    if (op == IC_OP_GET)
    {
        ...
        else if (attr_id == 4U)
        {
            ...
            {
                valid = valid && csm_array_write_buff(out, d->control_config,
                                                        d->control_config_len);
            }
            return valid ? CSM_OK : CSM_ERR_BAD_ENCODING;
        }
        else if (attr_id == 5U)
    ...
    else if (op == IC_OP_SET)
    {
        if (attr_id == 3U)
        {
            ...
            if (!csm_array_read_u8(in, &d->control_mode)) { return CSM_ERR_BAD_ENCODING; }
            return CSM_OK;
        }
        else if (attr_id == 4U)
        {
            ...
            if (!csm_array_read_u8(in, &len) || len > DISC_CONFIG_MAX) { return CSM_ERR_BAD_ENCODING; }
            d->control_config_len = len;
            if (len > 0U)
            {
                if (!csm_array_read_buff(in, d->control_config, len)) { return CSM_ERR_BAD_ENCODING; }
            }
            return CSM_OK;
        }
    }
