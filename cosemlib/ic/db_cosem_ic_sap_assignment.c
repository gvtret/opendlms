...
{
    (void) obis;
    if (sap_pool_count >= SAP_MAX_INSTANCES) { return NULL; }
...
static csm_db_code sap_dispatch(db_ic_inst_t *inst, db_ic_op_t op,
                                  ...
                                  csm_array *in, csm_array *out)
{
    ...
    if (op == IC_OP_GET)
    {
        ...
        else if (attr_id == 2U)
        {
            ...
            for (uint8_t i = 0U; i < d->entry_count && valid; i++)
            {
                ...
                {
                    valid = valid && csm_array_write_buff(out, d->entries[i].name,
                                                            d->entries[i].name_len);
                }
            }
            return valid ? CSM_OK : CSM_ERR_BAD_ENCODING;
        }
    ...
    else if (op == IC_OP_SET)
    {
        if (attr_id == 2U)
        {
            ...
            if (!csm_array_read_u8(in, &count) || count > SAP_MAX_ENTRIES) { return CSM_ERR_BAD_ENCODING; }
            d->entry_count = count;
            for (uint8_t i = 0U; i < count; i++)
            {
                ...
                uint8_t utag = 0xFFU;
                if (!csm_array_read_u8(in, &utag) || utag != AXDR_TAG_UNSIGNED16) { return CSM_ERR_BAD_ENCODING; }
                if (!csm_array_read_u16(in, &d->entries[i].sap_id)) { return CSM_ERR_BAD_ENCODING; }
                ...
                if (olen > 0U)
                {
                    if (!csm_array_read_buff(in, d->entries[i].name, olen)) { return CSM_ERR_BAD_ENCODING; }
                }
            }
            return CSM_OK;
        }
...
static const db_ic_class ic_sap = {
    .class_id  = 17,
    .name      = "SAP Assignment",
    .version   = 0,
    .create    = sap_create,
    .dispatch  = sap_dispatch
};
