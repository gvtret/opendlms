...
void db_ic_limiter_reset_count(void) {
	limiter_pool_count = 0U;
	limiter_inst_count = 0U;
}
...
static const db_ic_object_descr limiter_descr = {
    .attributes = limiter_attrs, .methods = limiter_methods, .class_id = 71, .obis = {0, 0, 96, 3, 11, 255},
               .attr_count = 11, .method_count = 1, .version = 0
};
...
static uint8_t lim_axdr_fixed_size(uint8_t tag) {
	switch (tag) {
		...
		case AXDR_TAG_ENUM:
			return 1U;
		case LIMITER_TAG_FLOAT32:
			return 4U;
		case LIMITER_TAG_FLOAT64:
			return 8U;
		default:
			return 0U;
	}
...
static int lim_read_axdr_value(csm_array *in, uint8_t *buf, uint8_t *len, uint8_t max_len) {
	uint8_t tag = 0xFFU;
...
static csm_db_code limiter_get_monitored_value(const db_ic_limiter_data *ld, csm_array *out) {
...
static csm_db_code limiter_set_monitored_value(db_ic_limiter_data *ld, csm_array *in) {
	uint8_t tag = 0xFFU;
	uint8_t flds = 0U;
...
static csm_db_code limiter_get_group_id_list(const db_ic_limiter_data *ld, csm_array *out) {
...
static csm_db_code limiter_set_group_id_list(db_ic_limiter_data *ld, csm_array *in) {
	uint8_t tag = 0xFFU;
...
static csm_db_code limiter_get_action_over(const db_ic_limiter_data *ld, csm_array *out) {
	int valid = csm_array_write_u8(out, AXDR_TAG_STRUCTURE);
	valid = valid && csm_array_write_u8(out, 2U);
	valid = valid && csm_array_write_u8(out, AXDR_TAG_UNSIGNED16);
	valid = valid && csm_array_write_u16(out, ld->action_script_id);
	valid = valid && csm_array_write_buff(out, ld->action_script_param_buf, ld->action_script_param_len);
	return valid ? CSM_OK : CSM_ERR_BAD_ENCODING;
}

static csm_db_code limiter_set_action_over(db_ic_limiter_data *ld, csm_array *in) {
	uint8_t tag = 0xFFU;
	uint8_t flds = 0U;
	if (!csm_array_read_u8(in, &tag) || tag != AXDR_TAG_STRUCTURE) {
		return CSM_ERR_BAD_ENCODING;
	...
	if (!lim_read_axdr_value(in, ld->action_script_param_buf, &ld->action_script_param_len, 32U)) {
		return CSM_ERR_BAD_ENCODING;
	}
...
static csm_db_code limiter_dispatch(db_ic_inst_t *inst, db_ic_op_t op, uint8_t attr_id, uint8_t method_id, csm_array *in, csm_array *out) {
	(void)method_id;
	...
	if (op == IC_OP_GET) {
		switch (attr_id) {
			...
			case 3U:
				return csm_array_write_buff(out, ld->threshold_active_buf, ld->threshold_active_len) ? CSM_OK : CSM_ERR_BAD_ENCODING;

			case 4U:
				return csm_array_write_buff(out, ld->threshold_normal_buf, ld->threshold_normal_len) ? CSM_OK : CSM_ERR_BAD_ENCODING;

			case 5U:
				return csm_array_write_buff(out, ld->threshold_emergency_buf, ld->threshold_emergency_len) ? CSM_OK : CSM_ERR_BAD_ENCODING;
		...
		switch (attr_id) {
			...
			case 3U:
				return lim_read_axdr_value(in, ld->threshold_active_buf, &ld->threshold_active_len, 32U) ? CSM_OK : CSM_ERR_BAD_ENCODING;

			case 4U:
				return lim_read_axdr_value(in, ld->threshold_normal_buf, &ld->threshold_normal_len, 32U) ? CSM_OK : CSM_ERR_BAD_ENCODING;

			case 5U:
				return lim_read_axdr_value(in, ld->threshold_emergency_buf, &ld->threshold_emergency_len, 32U) ? CSM_OK : CSM_ERR_BAD_ENCODING;
...
static const db_ic_class ic_limiter = {.class_id = 71, .name = "Limiter", .version = 0, .create = limiter_create, .dispatch = limiter_dispatch};
