...
void db_ic_profile_filter_reset_count(void) {
	prof_filter_pool_count = 0U;
}
...
static db_ic_inst_t *prof_filter_create(const csm_obis_code *obis) {
	(void)obis;
	if (prof_filter_pool_count >= PROF_FILTER_MAX_INSTANCES) {
		return NULL;
	}
...
static csm_db_code prof_filter_dispatch(db_ic_inst_t *inst, db_ic_op_t op, uint8_t attr_id, uint8_t method_id, csm_array *in, csm_array *out) {
	...
	if (op == IC_OP_GET) {
		...
		if (attr_id == 2U) {
			...
			if (!csm_array_read_u32(in, &d->filter_enable)) {
				return CSM_ERR_BAD_ENCODING;
			}
			return CSM_OK;
		} else if (attr_id == 3U) {
			...
			if (!csm_array_read_u8(in, &count) || count > PROF_FILTER_MAX_ENTRIES) {
				return CSM_ERR_BAD_ENCODING;
			}
			d->entry_count = count;
			for (uint8_t i = 0U; i < count; i++) {
				...
				if (!csm_array_read_u16(in, &d->entries[i].class_id)) {
					return CSM_ERR_BAD_ENCODING;
				}
				...
				if (!csm_array_read_u8(in, &d->entries[i].attribute_id)) {
					return CSM_ERR_BAD_ENCODING;
				}
			}
			return CSM_OK;
		...
		if (method_id == 1U) {
			uint32_t from = 0U;
			uint32_t count = 0U;
			if (!prof_filter_read_row_selector(in, &from, &count)) {
				return CSM_ERR_BAD_ENCODING;
			}

			if ((count > 0xFFU) || (from > d->entry_count) || (count > ((uint32_t)d->entry_count - from))) {
			...
			for (uint32_t i = 0U; valid && i < count; i++) {
				...
				csm_db_code rc = (csm_db_code)db_ic_dispatch(target, IC_OP_GET, entry->attribute_id, 0U, NULL, &value);
				if (rc != CSM_OK) {
					return rc;
				}
...
static const db_ic_class ic_prof_filter = {
    .class_id = 31, .name = "Profile Filter", .version = 0, .create = prof_filter_create, .dispatch = prof_filter_dispatch
};
