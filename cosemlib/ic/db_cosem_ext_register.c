...
void db_ic_ext_register_reset_count(void) {
	ext_val_count = 0U;
	ext_inst_count = 0U;
}
...
static uint8_t ext_axdr_fixed_size(uint8_t tag) {
	switch (tag) {
		...
		case AXDR_TAG_ENUM:
			return 1U;
		case EXT_TAG_FLOAT32:
			return 4U;
		case EXT_TAG_FLOAT64:
			return 8U;
		default:
			return 0U;
	}
...
static csm_db_code ext_dispatch(db_ic_inst_t *inst, db_ic_op_t op, uint8_t attr_id, uint8_t method_id, csm_array *in, csm_array *out) {
	(void)method_id;
...
static const db_ic_class ic_ext_register_class = {.class_id = 4, .name = "Extended Register", .version = 0, .create = ext_create, .dispatch = ext_dispatch};
