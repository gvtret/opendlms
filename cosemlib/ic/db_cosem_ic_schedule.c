...
void db_ic_schedule_reset_count(void) {
	schedule_data_count = 0U;
	schedule_inst_count = 0U;
}
...
static const db_ic_object_descr schedule_descr = {
    .attributes = schedule_attrs, .methods = schedule_methods, .class_id = 10, .obis = {0, 0, 10, 0, 0, 0},
               .attr_count = 2, .method_count = 4, .version = 0
};
...
static csm_db_code schedule_insert_entry(db_ic_schedule_data_t *data, csm_array *in) {
	...
	if ((entry_size == 0U) || (entry_size > SCHEDULE_ENTRY_SIZE) || !schedule_parse_entry_id(in, &entry_id)) {
...
static csm_db_code schedule_delete_entry(db_ic_schedule_data_t *data, csm_array *in) {
	...
	if (!csm_array_read_u8(in, &tag) || tag != AXDR_TAG_UNSIGNED16 || !csm_array_read_u16(in, &entry_id) || csm_array_unread(in) != 0U) {
		return CSM_ERR_BAD_ENCODING;
	}
...
static csm_db_code schedule_dispatch(db_ic_inst_t *inst, db_ic_op_t op, uint8_t attr_id, uint8_t method_id, csm_array *in, csm_array *out) {
...
static const db_ic_class ic_schedule_class = {.class_id = 10, .name = "Schedule", .version = 0, .create = schedule_create, .dispatch = schedule_dispatch};
