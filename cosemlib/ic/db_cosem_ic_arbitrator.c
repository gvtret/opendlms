...
void db_ic_arbitrator_reset_count(void) {
	arb_pool_count = 0U;
}
...
static const db_ic_object_descr arb_descr = {
    .attributes = arb_attrs, .methods = arb_methods, .class_id = 68, .obis = {0, 0, 60, 7, 0, 255},
               .attr_count = 6, .method_count = 2, .version = 0
};

static db_ic_inst_t *arb_create(const csm_obis_code *obis) {
	(void)obis;
	if (arb_pool_count >= ARB_MAX_INSTANCES) {
		return NULL;
	}
...
static int arb_read_structure_raw(csm_array *in, uint8_t *buf, uint8_t max_len, uint8_t *out_len) {
	...
	for (uint8_t f = 0U; f < flds; f++) {
		...
		if (ftag == AXDR_TAG_BOOLEAN || ftag == AXDR_TAG_UNSIGNED8 || ftag == AXDR_TAG_INTEGER8 || ftag == AXDR_TAG_ENUM) {
			...
			if (!csm_array_read_u8(in, &buf[pos++])) {
				return FALSE;
			}
...
static csm_db_code arb_dispatch(db_ic_inst_t *inst, db_ic_op_t op, uint8_t attr_id, uint8_t method_id, csm_array *in, csm_array *out) {
	...
	if (op == IC_OP_GET) {
		...
		if (attr_id == 2U) {
			...
			if (!csm_array_read_u32(in, &d->target_device_address)) {
				return CSM_ERR_BAD_ENCODING;
			}
			return CSM_OK;
		} else if (attr_id == 3U) {
			uint8_t len = 0U;
			if (!arb_read_structure_raw(in, d->power_limit_buf, ARB_LIMITER_BUF, &len)) {
				return CSM_ERR_BAD_ENCODING;
			}
			...
			if (!csm_array_read_u8(in, &len) || len > ARB_PHASE_MAX) {
				return CSM_ERR_BAD_ENCODING;
			}
			d->phase_list_len = len;
			if (len > 0U) {
				if (!csm_array_read_buff(in, d->current_phase_list, len)) {
					return CSM_ERR_BAD_ENCODING;
				}
			}
			return CSM_OK;
			...
			uint8_t len = 0U;
			if (!arb_read_structure_raw(in, d->group1_limiter_buf, ARB_LIMITER_BUF, &len)) {
				return CSM_ERR_BAD_ENCODING;
			}
			...
			uint8_t len = 0U;
			if (!arb_read_structure_raw(in, d->group2_limiter_buf, ARB_LIMITER_BUF, &len)) {
				return CSM_ERR_BAD_ENCODING;
			}
...
static const db_ic_class ic_arb = {.class_id = 68, .name = "Arbitrator", .version = 0, .create = arb_create, .dispatch = arb_dispatch};
