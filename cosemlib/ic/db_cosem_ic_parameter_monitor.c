...
void db_ic_parameter_monitor_reset_count(void) {
	param_mon_pool_count = 0U;
}
...
static const db_ic_object_descr param_mon_descr = {
    .attributes = param_mon_attrs, .methods = param_mon_methods, .class_id = 65, .obis = {0, 0, 60, 6, 0, 255},
               .attr_count = 5, .method_count = 2, .version = 0
};

static db_ic_inst_t *param_mon_create(const csm_obis_code *obis) {
	(void)obis;
	if (param_mon_pool_count >= PARAM_MON_MAX_INSTANCES) {
		return NULL;
	}
...
static int param_mon_read_entry(csm_array *in, db_ic_param_mon_entry *entry) {
	...
	if (!csm_array_read_u16(in, &entry->class_id)) {
		return FALSE;
	}
	...
	if (!csm_array_read_u8(in, &entry->attribute_id)) {
		return FALSE;
	}
	entry->data_len = 0U;
	return TRUE;
...
static csm_db_code param_mon_dispatch(db_ic_inst_t *inst, db_ic_op_t op, uint8_t attr_id, uint8_t method_id, csm_array *in, csm_array *out) {
	...
	if (op == IC_OP_GET) {
		...
		if (attr_id == 2U) {
			...
			if (!csm_array_read_u32(in, &d->current_page)) {
				return CSM_ERR_BAD_ENCODING;
			}
			return CSM_OK;
		} else if (attr_id == 3U) {
			...
			if (!csm_array_read_u32(in, &d->page_size)) {
				return CSM_ERR_BAD_ENCODING;
			}
			return CSM_OK;
		}
		...
		if (method_id == 1U) {
			...
			if (!param_mon_read_entry(in, &d->entries[d->entry_count])) {
				return CSM_ERR_BAD_ENCODING;
			}
			d->entry_count++;
			return CSM_OK;
...
static const db_ic_class ic_param_mon = {.class_id = 65, .name = "Parameter Monitor", .version = 0, .create = param_mon_create, .dispatch = param_mon_dispatch};
