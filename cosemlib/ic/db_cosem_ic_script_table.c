...
void db_ic_script_table_reset_count(void) {
	script_table_data_count = 0U;
	script_table_inst_count = 0U;
}
...
static csm_db_code script_table_dispatch(db_ic_inst_t *inst, db_ic_op_t op, uint8_t attr_id, uint8_t method_id, csm_array *in, csm_array *out) {
	(void)method_id;
	...
	if (op == IC_OP_GET) {
		...
		if (method_id == 1U) {
			...
			if (!csm_array_read_u8(in, &tag) || tag != AXDR_TAG_UNSIGNED16 || !csm_array_read_u16(in, &script_id) || csm_array_unread(in) != 0U) {
				return CSM_ERR_BAD_ENCODING;
			}

			for (uint8_t i = 0U; i < data->script_count; i++) {
				if (data->scripts[i][0] == (uint8_t)(script_id >> 8) && data->scripts[i][1] == (uint8_t)script_id) {
					return CSM_OK;
				}
			}
...
static const db_ic_class ic_script_table_class = {
    .class_id = 9, .name = "Script Table", .version = 0, .create = script_table_create, .dispatch = script_table_dispatch
};
