...
void db_ic_compact_data_reset_count(void) {
	compact_pool_count = 0U;
}
...
static const db_ic_object_descr compact_descr = {
    .attributes = compact_attrs, .methods = compact_methods, .class_id = 62, .obis = {0, 0, 60, 3, 0, 255},
               .attr_count = 6, .method_count = 2, .version = 0
};

static db_ic_inst_t *compact_create(const csm_obis_code *obis) {
	(void)obis;
	if (compact_pool_count >= COMPACT_MAX_INSTANCES) {
		return NULL;
	}
...
static csm_db_code compact_capture(db_ic_compact_data *d) {
	...
	for (uint8_t i = 0U; i < d->capture_count; i++) {
		...
		csm_db_code code = (csm_db_code)db_ic_dispatch(captured, IC_OP_GET, d->captures[i].attribute_id, 0U, &capture_in, &capture_out);
		if (code != CSM_OK) {
			return code;
		}
	}
...
static csm_db_code compact_dispatch(db_ic_inst_t *inst, db_ic_op_t op, uint8_t attr_id, uint8_t method_id, csm_array *in, csm_array *out) {
	...
	if (op == IC_OP_GET) {
		if (attr_id == 1U) {
			...
			return csm_axdr_wr_octetstring(out, d->buffer, d->buffer_len) ? CSM_OK : CSM_ERR_BAD_ENCODING;
		} else if (attr_id == 3U) {
		...
		if (attr_id == 2U) {
			uint32_t len = 0U;
			if (!csm_axdr_rd_octetstring(in, &len) || len > COMPACT_BUFFER_MAX) {
				return CSM_ERR_BAD_ENCODING;
			}
			d->buffer_len = (uint16_t)len;
			if (len > 0U) {
				if (!csm_array_read_buff(in, d->buffer, len)) {
					return CSM_ERR_BAD_ENCODING;
				}
			}
			return CSM_OK;
			...
			if (!csm_array_read_u8(in, &count) || count > COMPACT_MAX_CAPTURE) {
				return CSM_ERR_BAD_ENCODING;
			}
			...
			for (uint8_t i = 0U; i < count; i++) {
				...
				if (!csm_array_read_u16(in, &d->captures[i].class_id)) {
					return CSM_ERR_BAD_ENCODING;
				}
				...
				if (!csm_array_read_u8(in, &d->captures[i].attribute_id)) {
					return CSM_ERR_BAD_ENCODING;
				}
			}
			return CSM_OK;
			...
			if (!csm_array_read_u32(in, &d->capture_period)) {
				return CSM_ERR_BAD_ENCODING;
			}
			return CSM_OK;
		}
...
static const db_ic_class ic_compact = {.class_id = 62, .name = "Compact Data", .version = 0, .create = compact_create, .dispatch = compact_dispatch};
