...
void db_ic_single_action_reset_count(void) {
	single_action_data_count = 0U;
	single_action_inst_count = 0U;
}
...
static csm_db_code single_action_read_raw(csm_array *in, uint8_t *buf, uint8_t *len, uint8_t max_len) {
...
static csm_db_code single_action_dispatch(db_ic_inst_t *inst, db_ic_op_t op, uint8_t attr_id, uint8_t method_id, csm_array *in, csm_array *out) {
	(void)method_id;
	...
	if (op == IC_OP_GET) {
		...
		if (attr_id == 2U) {
			return single_action_read_raw(in, data->executed_script, &data->script_len, SINGLE_ACTION_SCRIPT_SIZE);
		} else if (attr_id == 3U) {
			uint8_t tag = 0U;
			uint8_t type = 0U;
			if (!csm_array_read_u8(in, &tag) || tag != AXDR_TAG_ENUM || !csm_array_read_u8(in, &type) || csm_array_unread(in) != 0U) {
				return CSM_ERR_BAD_ENCODING;
			}
			data->type = type;
			return CSM_OK;
		} else if (attr_id == 4U) {
			return single_action_read_raw(in, data->execution_time, &data->time_len, SINGLE_ACTION_TIME_SIZE);
		}
	}
...
static const db_ic_class ic_single_action_class = {
    .class_id = 22, .name = "Single Action Schedule", .version = 0, .create = single_action_create, .dispatch = single_action_dispatch
};
