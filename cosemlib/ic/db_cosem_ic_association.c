...
static uint8_t aso_axdr_fixed_size(uint8_t tag)
{
    ...
    case ASO_TAG_FLOAT32:     return 4U;
    case ASO_TAG_FLOAT64:     return 8U;
    default:                  return 0U;
    }
}
...
static int aso_read_axdr_value(csm_array *in, uint8_t *buf, uint8_t *len,
                                uint8_t max_len)
{
    uint8_t tag = 0xFFU;
...
static int aso_read_octetstring(csm_array *in, uint8_t *buf, uint8_t *len,
                                 uint8_t max_len)
{
    uint8_t tag = 0xFFU;
...
static csm_db_code aso_get_user_list(const db_ic_aso_data *ad, csm_array *out)
{
    ...
    for (uint8_t i = 0U; i < ad->user_count && valid; i++)
    {
        ...
        {
            valid = valid && csm_array_write_buff(out, ad->users[i].name,
                                                    ad->users[i].name_len);
        }
        valid = valid && csm_array_write_u8(out, AXDR_TAG_OCTETSTRING);
        ...
        {
            valid = valid && csm_array_write_buff(out, ad->users[i].password,
                                                    ad->users[i].password_len);
        }
    }
...
static int aso_find_object(const db_ic_aso_data *ad, uint16_t class_id,
                            const csm_obis_code *obis)
{
    for (uint8_t i = 0U; i < ad->object_count; i++)
    {
        if (ad->objects[i].class_id == class_id &&
            ad->objects[i].obis.A == obis->A &&
            ad->objects[i].obis.B == obis->B &&
            ad->objects[i].obis.C == obis->C &&
            ad->objects[i].obis.D == obis->D &&
            ad->objects[i].obis.E == obis->E &&
            ad->objects[i].obis.F == obis->F)
        {
            return (int)i;
        }
    }
...
static int aso_find_user(const db_ic_aso_data *ad, const uint8_t *name,
                           uint8_t name_len)
{
    for (uint8_t i = 0U; i < ad->user_count; i++)
    {
        if (ad->users[i].name_len == name_len &&
            memcmp(ad->users[i].name, name, name_len) == 0)
        {
            return (int)i;
        }
    }
...
static csm_db_code aso_dispatch(db_ic_inst_t *inst, db_ic_op_t op,
                                  ...
                                  csm_array *in, csm_array *out)
{
    ...
    if (op == IC_OP_GET)
    {
        ...
        case 3U:
            return csm_array_write_buff(out, ad->access_rights_buf,
                                          ad->access_rights_len)
        ...
        case 4U:
        {
            ...
            {
                valid = valid && csm_array_write_buff(out, ad->app_context_buf,
                                                        ad->app_context_len);
            }
            return valid ? CSM_OK : CSM_ERR_BAD_ENCODING;
        }
        ...
        case 5U:
        {
            ...
            {
                valid = valid && csm_array_write_buff(out, ad->sec_mechanism_buf,
                                                        ad->sec_mechanism_len);
            }
            return valid ? CSM_OK : CSM_ERR_BAD_ENCODING;
        }
        ...
        case 7U:
            return csm_array_write_buff(out, ad->xdlms_ctx_buf,
                                          ad->xdlms_ctx_len)
                   ? CSM_OK : CSM_ERR_BAD_ENCODING;
        ...
        case 8U:
        {
            ...
            {
                valid = valid && csm_array_write_buff(out, ad->auth_mechanism_buf,
                                                        ad->auth_mechanism_len);
            }
            return valid ? CSM_OK : CSM_ERR_BAD_ENCODING;
        }
        ...
        case 9U:
        {
            ...
            if (ad->secret_len > 0U)
            {
                valid = valid && csm_array_write_buff(out, ad->secret_buf,
                                                        ad->secret_len);
            }
            return valid ? CSM_OK : CSM_ERR_BAD_ENCODING;
        }
        ...
        case 11U:
        {
            ...
            {
                valid = valid && csm_array_write_buff(out, ad->current_user_buf,
                                                        ad->current_user_len);
            }
            return valid ? CSM_OK : CSM_ERR_BAD_ENCODING;
        }
    ...
    else if (op == IC_OP_SET)
    {
        ...
        case 3U:
            return aso_read_axdr_value(in, ad->access_rights_buf,
                                        &ad->access_rights_len, ASO_MAX_ACCESS_SIZE)
                   ? CSM_OK : CSM_ERR_BAD_ENCODING;

        case 9U:
            return aso_read_axdr_value(in, ad->secret_buf,
                                        &ad->secret_len, ASO_MAX_SECRET_SIZE)
                   ? CSM_OK : CSM_ERR_BAD_ENCODING;
    ...
    else if (op == IC_OP_ACTION)
    {
        ...
        case 6U:
            return aso_method_change_password(ad, in);
...
static const db_ic_class ic_asso_ln = {
    .class_id  = 15,
    .name      = "Association LN",
    .version   = 0,
    .create    = aso_create,
    .dispatch  = aso_dispatch
};
