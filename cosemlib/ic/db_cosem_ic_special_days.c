...
void db_ic_special_days_reset_count(void) {
	special_days_data_count = 0U;
	special_days_inst_count = 0U;
}
...
static csm_db_code special_days_insert_entry(db_ic_special_days_data_t *data, csm_array *in) {
	...
	if ((entry_size == 0U) || (entry_size > SPECIAL_DAYS_ENTRY_SIZE) || !special_days_parse_day_id(in, &day_id)) {
...
static csm_db_code special_days_delete_entry(db_ic_special_days_data_t *data, csm_array *in) {
	...
	if (!csm_array_read_u8(in, &tag) || tag != AXDR_TAG_UNSIGNED16 || !csm_array_read_u16(in, &day_id) || csm_array_unread(in) != 0U) {
		return CSM_ERR_BAD_ENCODING;
	}
	...
	for (uint8_t i = 0U; i < data->entry_count; i++) {
		if (data->day_id[i] == day_id) {
			for (uint8_t j = i; (j + 1U) < data->entry_count; j++) {
				...
				memcpy(data->entries[j], data->entries[j + 1U], SPECIAL_DAYS_ENTRY_SIZE);
			}
			data->entry_count--;
			return CSM_OK;
		}
...
static csm_db_code special_days_dispatch(db_ic_inst_t *inst, db_ic_op_t op, uint8_t attr_id, uint8_t method_id, csm_array *in, csm_array *out) {
...
static const db_ic_class ic_special_days_class = {
    .class_id = 11, .name = "Special Days", .version = 0, .create = special_days_create, .dispatch = special_days_dispatch
};
