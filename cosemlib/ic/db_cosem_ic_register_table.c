...
void db_ic_register_table_reset_count(void) {
	reg_table_pool_count = 0U;
}
...
static const db_ic_object_descr reg_table_descr = {
    .attributes = reg_table_attrs, .methods = reg_table_methods, .class_id = 61, .obis = {0, 0, 60, 2, 0, 255},
               .attr_count = 4, .method_count = 2, .version = 0
};

static db_ic_inst_t *reg_table_create(const csm_obis_code *obis) {
	(void)obis;
	if (reg_table_pool_count >= REG_TABLE_MAX_INSTANCES) {
		return NULL;
	}
...
static csm_db_code reg_table_dispatch(db_ic_inst_t *inst, db_ic_op_t op, uint8_t attr_id, uint8_t method_id, csm_array *in, csm_array *out) {
	...
	if (op == IC_OP_GET) {
		...
		if (attr_id == 2U) {
			...
			if (!csm_array_read_u8(in, &count) || count > REG_TABLE_MAX_ENTRIES) {
				return CSM_ERR_BAD_ENCODING;
			}
			d->entry_count = count;
			...
			if (!csm_array_read_u16(in, &d->class_id)) {
				return CSM_ERR_BAD_ENCODING;
			}
			return CSM_OK;
		} else if (attr_id == 4U) {
			...
			if (!csm_array_read_u16(in, &d->active_index)) {
				return CSM_ERR_BAD_ENCODING;
			}
			return CSM_OK;
		}
		...
		if (method_id == 1U) {
			...
			if ((d->class_id == 0U) || (d->entry_count == 0U) || (d->active_index >= d->entry_count)) {
			...
			return (csm_db_code)db_ic_dispatch(target, IC_OP_GET, entry->attribute_id, 0U, NULL, &capture_out);
		}
	}
	return CSM_ERR_OBJECT_NOT_FOUND;
}

static const db_ic_class ic_reg_table = {.class_id = 61, .name = "Register Table", .version = 0, .create = reg_table_create, .dispatch = reg_table_dispatch};
