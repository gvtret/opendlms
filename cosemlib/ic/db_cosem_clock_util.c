...
static uint32_t epoch_from_broken(uint16_t year, uint8_t month, uint8_t day,
...
static void broken_from_epoch(uint32_t epoch, uint16_t *year, uint8_t *month,
...
void db_cosem_clock_set_epoch(uint32_t seconds)
{
    ...
    CSM_LOG("[Clock] Epoch %lu = %u-%02u-%02u %02u:%02u:%02u",
            (unsigned long)seconds, year, month, day, hour, minute, second);
...
uint32_t db_cosem_clock_get_epoch(const db_ic_inst_t *inst)
{
    ...
    return epoch_from_broken(clk->year, clk->month, clk->day,
