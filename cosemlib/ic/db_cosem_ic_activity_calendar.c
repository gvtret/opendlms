...
void db_ic_activity_cal_reset_count(void) {
	activity_cal_data_count = 0U;
	activity_cal_inst_count = 0U;
}
...
static csm_db_code activity_cal_dispatch(db_ic_inst_t *inst, db_ic_op_t op, uint8_t attr_id, uint8_t method_id, csm_array *in, csm_array *out) {
	(void)method_id;
	...
	if (op == IC_OP_GET) {
		...
		if (attr_id == 2U) {
			...
			calendar[1] = len;
			if (len > 0U && !csm_array_read_buff(in, &calendar[2], len)) {
				return CSM_ERR_BAD_ENCODING;
			}
...
static const db_ic_class ic_activity_cal_class = {
    .class_id = 20, .name = "Activity Calendar", .version = 0, .create = activity_cal_create, .dispatch = activity_cal_dispatch
};
