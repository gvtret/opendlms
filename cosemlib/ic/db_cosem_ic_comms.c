...
static const db_ic_object_descr iec_local_descr = {
    .attributes = iec_local_attrs, .methods = NULL, .class_id = 19, .obis = {0, 0, 10, 0, 0, 255},
               .attr_count = 2, .method_count = 0, .version = 0
};

static db_ic_inst_t *iec_local_create(const csm_obis_code *obis) {
	(void)obis;
	if (iec_local_count >= COMMS_MAX_INSTANCES) {
		return NULL;
	}
...
static csm_db_code iec_local_dispatch(db_ic_inst_t *inst, db_ic_op_t op, uint8_t attr_id, uint8_t method_id, csm_array *in, csm_array *out) {
	(void)method_id;
...
static const db_ic_class ic_iec_local = {
    .class_id = 19, .name = "IEC Local Port Setup", .version = 0, .create = iec_local_create, .dispatch = iec_local_dispatch
};

void db_ic_register_iec_local_port_setup(void) {
	db_ic_register(&ic_iec_local);
}
...
static const db_ic_object_descr iec_hdlc_descr = {
    .attributes = iec_hdlc_attrs, .methods = NULL, .class_id = 23, .obis = {0, 0, 22, 0, 0, 255},
               .attr_count = 5, .method_count = 0, .version = 0
};

static db_ic_inst_t *iec_hdlc_create(const csm_obis_code *obis) {
	(void)obis;
	if (iec_hdlc_count >= COMMS_MAX_INSTANCES) {
		return NULL;
	}
...
static csm_db_code iec_hdlc_dispatch(db_ic_inst_t *inst, db_ic_op_t op, uint8_t attr_id, uint8_t method_id, csm_array *in, csm_array *out) {
	(void)method_id;
	if ((inst == NULL) || (inst->data == NULL)) {
		return CSM_ERR_OBJECT_NOT_FOUND;
	}
...
static const db_ic_class ic_iec_hdlc = {.class_id = 23, .name = "IEC HDLC Setup", .version = 0, .create = iec_hdlc_create, .dispatch = iec_hdlc_dispatch};

void db_ic_register_iec_hdlc_setup(void) {
	db_ic_register(&ic_iec_hdlc);
}
...
static const db_ic_object_descr tcp_udp_descr = {
    .attributes = tcp_udp_attrs, .methods = NULL, .class_id = 41, .obis = {0, 0, 25, 0, 0, 255},
               .attr_count = 7, .method_count = 0, .version = 0
};

static db_ic_inst_t *tcp_udp_create(const csm_obis_code *obis) {
	(void)obis;
	if (tcp_udp_count >= COMMS_MAX_INSTANCES) {
		return NULL;
	}
...
static int comms_read_octet_string(csm_array *in, uint8_t *buf, uint8_t max_len, uint8_t *out_len) {
	...
	if ((buf == NULL) || (out_len == NULL) || (max_len > COMMS_MAC_MAX) || !csm_array_read_u8(in, &len) || len > max_len) {
		return FALSE;
	}
	if (len > 0U && !csm_array_read_buff(in, tmp, len)) {
		return FALSE;
	}
...
static csm_db_code tcp_udp_dispatch(db_ic_inst_t *inst, db_ic_op_t op, uint8_t attr_id, uint8_t method_id, csm_array *in, csm_array *out) {
	(void)method_id;
	if ((inst == NULL) || (inst->data == NULL)) {
		return CSM_ERR_OBJECT_NOT_FOUND;
	}
	...
	if (op == IC_OP_GET) {
		...
		if (attr_id == 2U) {
			uint8_t len = 0U;
			if (!comms_read_octet_string(in, d->ip_address, COMMS_IP_LEN, &len)) {
				return CSM_ERR_BAD_ENCODING;
			}
			return CSM_OK;
		} else if (attr_id == 3U) {
			uint8_t len = 0U;
			if (!comms_read_octet_string(in, d->subnet_mask, COMMS_IP_LEN, &len)) {
				return CSM_ERR_BAD_ENCODING;
			}
			return CSM_OK;
		} else if (attr_id == 4U) {
			uint8_t len = 0U;
			if (!comms_read_octet_string(in, d->gateway_ip, COMMS_IP_LEN, &len)) {
				return CSM_ERR_BAD_ENCODING;
			}
			return CSM_OK;
		} else if (attr_id == 5U) {
			...
			uint8_t len = 0U;
			if (!comms_read_octet_string(in, d->primary_dns, COMMS_IP_LEN, &len)) {
				return CSM_ERR_BAD_ENCODING;
			}
			return CSM_OK;
		} else if (attr_id == 7U) {
			uint8_t len = 0U;
			if (!comms_read_octet_string(in, d->secondary_dns, COMMS_IP_LEN, &len)) {
				return CSM_ERR_BAD_ENCODING;
			}
			return CSM_OK;
		}
...
static const db_ic_class ic_tcp_udp = {.class_id = 41, .name = "TCP-UDP Setup", .version = 0, .create = tcp_udp_create, .dispatch = tcp_udp_dispatch};

void db_ic_register_tcp_udp_setup(void) {
	db_ic_register(&ic_tcp_udp);
}
