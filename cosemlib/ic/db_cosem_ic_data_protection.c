...
{
    (void) obis;
    if (data_prot_pool_count >= DATA_PROT_MAX_INSTANCES) { return NULL; }
...
static csm_db_code data_prot_dispatch(db_ic_inst_t *inst, db_ic_op_t op,
                                        ...
                                        csm_array *in, csm_array *out)
{
    ...
    if (op == IC_OP_GET)
    {
        ...
        else if (attr_id == 2U)
        {
            ...
            if (d->key_len > 0U) { valid = valid && csm_array_write_buff(out, d->key, d->key_len); }
            return valid ? CSM_OK : CSM_ERR_BAD_ENCODING;
        }
        else if (attr_id == 3U)
        {
            ...
            if (d->password_len > 0U) { valid = valid && csm_array_write_buff(out, d->password, d->password_len); }
            return valid ? CSM_OK : CSM_ERR_BAD_ENCODING;
        }
        else if (attr_id == 4U)
        {
    ...
    else if (op == IC_OP_SET)
    {
        if (attr_id == 2U)
        {
            ...
            if (!csm_array_read_u8(in, &len) || len > DATA_PROT_KEY_MAX) { return CSM_ERR_BAD_ENCODING; }
            d->key_len = len;
            if (len > 0U)
            {
                if (!csm_array_read_buff(in, d->key, len)) { return CSM_ERR_BAD_ENCODING; }
            }
            return CSM_OK;
        }
        else if (attr_id == 3U)
        {
            ...
            uint8_t len = 0U;
            if (!csm_array_read_u8(in, &tag) || tag != AXDR_TAG_OCTETSTRING) { return CSM_ERR_BAD_ENCODING; }
            if (!csm_array_read_u8(in, &len) || len > DATA_PROT_PWD_MAX) { return CSM_ERR_BAD_ENCODING; }
            d->password_len = len;
            if (len > 0U)
            {
                if (!csm_array_read_buff(in, d->password, len)) { return CSM_ERR_BAD_ENCODING; }
            }
            return CSM_OK;
        }
    }
...
static const db_ic_class ic_data_prot = {
    .class_id  = 30,
    .name      = "Data Protection",
    .version   = 0,
    .create    = data_prot_create,
    .dispatch  = data_prot_dispatch
};
