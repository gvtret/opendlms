...
void db_ic_table_manager_reset_count(void) {
	table_mgr_pool_count = 0U;
}
...
static db_ic_inst_t *table_mgr_create(const csm_obis_code *obis) {
	(void)obis;
	if (table_mgr_pool_count >= TABLE_MGR_MAX_INSTANCES) {
		return NULL;
	}
...
static int table_mgr_read_row_selector(csm_array *in, uint32_t *from, uint32_t *count) {
	...
	if (!csm_array_read_u32(in, from)) {
		return FALSE;
	}
	if (!csm_array_read_u8(in, &tag) || tag != AXDR_TAG_UNSIGNED32) {
		return FALSE;
	}
	if (!csm_array_read_u32(in, count)) {
		return FALSE;
	}
	return TRUE;
}

static csm_db_code table_mgr_dispatch(db_ic_inst_t *inst, db_ic_op_t op, uint8_t attr_id, uint8_t method_id, csm_array *in, csm_array *out) {
	...
	if (op == IC_OP_GET) {
		if (attr_id == 1U) {
			...
			for (uint8_t i = 0U; i < d->table_count && valid; i++) {
				...
				if (d->tables[i].name_len > 0U) {
					valid = valid && csm_array_write_buff(out, d->tables[i].name, d->tables[i].name_len);
				}
			}
			return valid ? CSM_OK : CSM_ERR_BAD_ENCODING;
		} else if (attr_id == 3U) {
			...
			return csm_axdr_wr_octetstring(out, d->table_data, d->table_data_len) ? CSM_OK : CSM_ERR_BAD_ENCODING;
		}
		...
		if (attr_id == 2U) {
			...
			if (!csm_array_read_u8(in, &count) || count > TABLE_MGR_MAX_TABLES) {
				return CSM_ERR_BAD_ENCODING;
			}
			d->table_count = count;
			for (uint8_t i = 0U; i < count; i++) {
				...
				uint8_t utag = 0xFFU;
				if (!csm_array_read_u8(in, &utag) || utag != AXDR_TAG_UNSIGNED32) {
					return CSM_ERR_BAD_ENCODING;
				}
				if (!csm_array_read_u32(in, &d->tables[i].table_id)) {
					return CSM_ERR_BAD_ENCODING;
				}
				...
				if (olen > 0U) {
					if (!csm_array_read_buff(in, d->tables[i].name, olen)) {
						return CSM_ERR_BAD_ENCODING;
					}
				}
			}
			...
			if (!csm_array_read_u32(in, &d->active_table_id)) {
				return CSM_ERR_BAD_ENCODING;
			}
			return CSM_OK;
		} else if (attr_id == 4U) {
			uint32_t len = 0U;
			if (!csm_axdr_rd_octetstring(in, &len) || len > TABLE_MGR_DATA_MAX) {
				return CSM_ERR_BAD_ENCODING;
			}
			d->table_data_len = (uint16_t)len;
			if (len > 0U) {
				if (!csm_array_read_buff(in, d->table_data, len)) {
					return CSM_ERR_BAD_ENCODING;
				}
			}
			return CSM_OK;
		...
		if (method_id == 1U) {
			uint32_t from = 0U;
			uint32_t count = 0U;
			if (!table_mgr_read_row_selector(in, &from, &count)) {
				return CSM_ERR_BAD_ENCODING;
			}

			if ((count > 0xFFU) || (from > d->table_data_len) || (count > ((uint32_t)d->table_data_len - from))) {
...
static const db_ic_class ic_table_mgr = {.class_id = 8200, .name = "Table Manager", .version = 0, .create = table_mgr_create, .dispatch = table_mgr_dispatch};
