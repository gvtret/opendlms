...
void db_ic_data_reset_count(void) {
	data_val_count = 0U;
	data_inst_count = 0U;
}
...
static const db_ic_object_descr data_descr = {
    .attributes = data_attrs, .methods = data_methods, .class_id = 1, .obis = {0, 0, 0, 0, 0, 0},
...
static uint8_t axdr_fixed_data_size(uint8_t tag) {
	switch (tag) {
		...
		case AXDR_TAG_ENUM:
			return 1U;
		case DATA_TAG_FLOAT32:
			return 4U;
		case DATA_TAG_FLOAT64:
			return 8U;
		default:
			return 0U;
	}
...
static csm_db_code data_dispatch(db_ic_inst_t *inst, db_ic_op_t op, uint8_t attr_id, uint8_t method_id, csm_array *in, csm_array *out) {
	(void)method_id;
...
static const db_ic_class ic_data_class = {.class_id = 1, .name = "Data", .version = 0, .create = data_create, .dispatch = data_dispatch};
