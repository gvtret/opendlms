...
void db_ic_push_reset_count(void) {
	push_pool_count = 0U;
	push_inst_count = 0U;
}
...
static const db_ic_object_descr push_descr = {
    .attributes = push_attrs, .methods = push_methods, .class_id = 40, .obis = {0, 0, 25, 1, 0, 255},
               .attr_count = 7, .method_count = 2, .version = 1
};
...
static uint8_t push_axdr_fixed_size(uint8_t tag) {
	switch (tag) {
		...
		case AXDR_TAG_ENUM:
			return 1U;
		case PUSH_TAG_FLOAT32:
			return 4U;
		case PUSH_TAG_FLOAT64:
			return 8U;
		default:
			return 0U;
	}
...
static csm_db_code push_get_repetition_delay(const db_ic_push_data *pd, csm_array *out) {
	int valid = csm_array_write_u8(out, AXDR_TAG_STRUCTURE);
	valid = valid && csm_array_write_u8(out, 2U);
	valid = valid && csm_array_write_u8(out, AXDR_TAG_UNSIGNED8);
...
static csm_db_code push_dispatch(db_ic_inst_t *inst, db_ic_op_t op, uint8_t attr_id, uint8_t method_id, csm_array *in, csm_array *out) {
	(void)method_id;
...
static const db_ic_class ic_push = {.class_id = 40, .name = "Push Setup", .version = 1, .create = push_create, .dispatch = push_dispatch};
