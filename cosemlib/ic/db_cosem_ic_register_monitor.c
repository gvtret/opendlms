...
void db_ic_reg_monitor_reset_count(void) {
	reg_monitor_data_count = 0U;
	reg_monitor_inst_count = 0U;
}
...
static csm_db_code reg_monitor_read_axdr_value(csm_array *in, uint8_t *buf, uint8_t *len, uint8_t max_len) {
	uint8_t tag = 0U;
	...
	if ((total_len == 0U) || (total_len > max_len) || (csm_array_unread(in) < total_len)) {
		return CSM_ERR_BAD_ENCODING;
	}
...
static csm_db_code reg_monitor_dispatch(db_ic_inst_t *inst, db_ic_op_t op, uint8_t attr_id, uint8_t method_id, csm_array *in, csm_array *out) {
	(void)method_id;
	...
	if (op == IC_OP_GET) {
		...
		if (attr_id == 2U) {
			uint8_t tag = 0U;
			uint8_t count = 0U;
			if (!csm_array_read_u8(in, &tag) || tag != AXDR_TAG_ARRAY || !csm_array_read_u8(in, &count) || count > REG_MONITOR_MAX_THRESHOLDS) {
				return CSM_ERR_BAD_ENCODING;
			}

			data->threshold_count = count;
			for (uint8_t i = 0U; i < count; i++) {
				csm_db_code rc = reg_monitor_read_axdr_value(in, data->thresholds[i], &data->threshold_len[i], REG_MONITOR_VALUE_SIZE);
				if (rc != CSM_OK) {
					return rc;
				}
			}
			...
			csm_db_code rc = reg_monitor_read_axdr_value(in, data->monitored_value, &data->monitored_len, REG_MONITOR_VALUE_SIZE);
			return (rc == CSM_OK && csm_array_unread(in) == 0U) ? CSM_OK : CSM_ERR_BAD_ENCODING;
		} else if (attr_id == 4U) {
			uint8_t tag = 0U;
			uint8_t count = 0U;
			if (!csm_array_read_u8(in, &tag) || tag != AXDR_TAG_ARRAY || !csm_array_read_u8(in, &count) || count > REG_MONITOR_MAX_ACTIONS) {
				return CSM_ERR_BAD_ENCODING;
			}

			data->action_count = count;
			for (uint8_t i = 0U; i < count; i++) {
				csm_db_code rc = reg_monitor_read_axdr_value(in, data->actions[i], &data->action_len[i], REG_MONITOR_ACTION_SIZE);
				if (rc != CSM_OK) {
					return rc;
				}
			}
...
static const db_ic_class ic_reg_monitor_class = {
    .class_id = 21, .name = "Register Monitor", .version = 0, .create = reg_monitor_create, .dispatch = reg_monitor_dispatch
};
