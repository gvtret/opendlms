...
void db_ic_sensor_manager_reset_count(void) {
	sens_mgr_pool_count = 0U;
}
...
static const db_ic_object_descr sens_mgr_descr = {
    .attributes = sens_mgr_attrs, .methods = sens_mgr_methods, .class_id = 67, .obis = {0, 0, 60, 8, 0, 255},
               .attr_count = 15, .method_count = 1, .version = 0
};

static db_ic_inst_t *sens_mgr_create(const csm_obis_code *obis) {
	(void)obis;
	if (sens_mgr_pool_count >= SENS_MGR_MAX_INSTANCES) {
		return NULL;
	}
...
static csm_db_code sens_mgr_dispatch(db_ic_inst_t *inst, db_ic_op_t op, uint8_t attr_id, uint8_t method_id, csm_array *in, csm_array *out) {
	...
	if (op == IC_OP_GET) {
		switch (attr_id) {
			...
			case 12U: {
				...
				if (d->key_len > 0U) {
					valid = valid && csm_array_write_buff(out, d->data_protection_key, d->key_len);
				}
				return valid ? CSM_OK : CSM_ERR_BAD_ENCODING;
			}
			case 13U: {
				...
				if (d->password_len > 0U) {
					valid = valid && csm_array_write_buff(out, d->data_protection_password, d->password_len);
				}
				return valid ? CSM_OK : CSM_ERR_BAD_ENCODING;
			}
			case 14U: {
		...
		switch (attr_id) {
			case 2U: {
				...
				if (!csm_array_read_u32(in, &d->active_variant)) {
					return CSM_ERR_BAD_ENCODING;
				}
				return CSM_OK;
			}
			case 3U: {
				...
				if (!csm_array_read_u32(in, &d->active_schedule)) {
					return CSM_ERR_BAD_ENCODING;
				}
				return CSM_OK;
			}
			case 5U: {
				...
				if (!csm_array_read_u8(in, &d->number_of_retries)) {
					return CSM_ERR_BAD_ENCODING;
				}
				return CSM_OK;
			}
			case 6U: {
				...
				if (!csm_array_read_u16(in, &d->retry_delay_time)) {
					return CSM_ERR_BAD_ENCODING;
				}
				return CSM_OK;
			}
			case 11U: {
				...
				if (!csm_array_read_u8(in, &d->data_protected)) {
					return CSM_ERR_BAD_ENCODING;
				}
				return CSM_OK;
			}
...
static const db_ic_class ic_sens_mgr = {.class_id = 67, .name = "Sensor Manager", .version = 0, .create = sens_mgr_create, .dispatch = sens_mgr_dispatch};
