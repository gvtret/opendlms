...
void db_ic_utility_tables_reset_count(void) { utility_pool_count = 0U; }
...
{
    (void) obis;
    if (utility_pool_count >= UTILITY_MAX_INSTANCES) { return NULL; }
...
static csm_db_code utility_dispatch(db_ic_inst_t *inst, db_ic_op_t op,
                                      ...
                                      csm_array *in, csm_array *out)
{
    ...
    if (op == IC_OP_GET)
    {
        ...
        {
            return csm_axdr_wr_octetstring(out, d->table_buf, d->table_len)
                ? CSM_OK
    ...
    else if (op == IC_OP_SET)
    {
        if (attr_id == 2U)
        {
            ...
            if (!csm_array_read_u32(in, &d->table_id)) { return CSM_ERR_BAD_ENCODING; }
            return CSM_OK;
        }
        else if (attr_id == 3U)
        {
            uint32_t len = 0U;
            if (!csm_axdr_rd_octetstring(in, &len) || len > UTILITY_TABLE_MAX) { return CSM_ERR_BAD_ENCODING; }
            d->table_len = (uint16_t)len;
            if (len > 0U)
            {
                if (!csm_array_read_buff(in, d->table_buf, len)) { return CSM_ERR_BAD_ENCODING; }
            }
            return CSM_OK;
        }
    }
...
static const db_ic_class ic_utility = {
    .class_id  = 26,
    .name      = "Utility Tables",
    .version   = 0,
    .create    = utility_create,
    .dispatch  = utility_dispatch
};
