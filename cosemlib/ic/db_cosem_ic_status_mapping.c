...
void db_ic_status_mapping_reset_count(void) {
	status_pool_count = 0U;
}
...
static const db_ic_object_descr status_descr = {
    .attributes = status_attrs, .methods = NULL, .class_id = 63, .obis = {0, 0, 60, 4, 0, 255},
               .attr_count = 3, .method_count = 0, .version = 0
};

static db_ic_inst_t *status_create(const csm_obis_code *obis) {
	(void)obis;
	if (status_pool_count >= STATUS_MAP_MAX_INSTANCES) {
		return NULL;
	}
...
static csm_db_code status_dispatch(db_ic_inst_t *inst, db_ic_op_t op, uint8_t attr_id, uint8_t method_id, csm_array *in, csm_array *out) {
	(void)method_id;
	...
	if (op == IC_OP_GET) {
		...
		if (attr_id == 2U) {
			...
			if (!csm_array_read_u8(in, &len) || len > STATUS_MAP_MAX_SIZE) {
				return CSM_ERR_BAD_ENCODING;
			}
			d->mapping_len = len;
			if (len > 0U) {
				if (!csm_array_read_buff(in, d->status_mapping, len)) {
					return CSM_ERR_BAD_ENCODING;
				}
			}
			return CSM_OK;
			...
			d->mask_len = len;
			if (len > 0U) {
				if (!csm_array_read_buff(in, d->status_mask, len)) {
					return CSM_ERR_BAD_ENCODING;
				}
			}
			return CSM_OK;
...
static const db_ic_class ic_status = {.class_id = 63, .name = "Status Mapping", .version = 0, .create = status_create, .dispatch = status_dispatch};
