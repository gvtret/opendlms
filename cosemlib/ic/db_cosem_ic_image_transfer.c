...
void db_ic_image_transfer_reset_count(void) {
	image_pool_count = 0U;
}
...
static const db_ic_object_descr image_descr = {
    .attributes = image_attrs, .methods = image_methods, .class_id = 18, .obis = {0, 0, 44, 0, 0, 255},
               .attr_count = 7, .method_count = 6, .version = 0
};

static db_ic_inst_t *image_create(const csm_obis_code *obis) {
	(void)obis;
	if (image_pool_count >= IMG_MAX_INSTANCES) {
		return NULL;
	}
...
static int image_read_init(csm_array *in, db_ic_image_data *d) {
	...
	if (!csm_array_read_u8(in, &fields) || fields != 2U) {
		return FALSE;
	}
	...
	if (identifier_len > 0U) {
		if (!csm_array_read_buff(in, d->image_identifier, identifier_len)) {
			return FALSE;
		}
	}
	...
	if (!csm_array_read_u32(in, &d->image_size)) {
		return FALSE;
	}
	if (d->block_size == 0U) {
		return FALSE;
	...
	if (status_len > IMG_BLOCK_STATUS_MAX) {
		return FALSE;
	}
...
static int image_all_blocks_transferred(const db_ic_image_data *d) {
	...
	return (d->blocks_transferred >= block_count) && (d->first_not_transferred >= block_count);
}
...
static int image_read_block_transfer(csm_array *in, db_ic_image_data *d, uint32_t *block_number) {
	...
	if (!csm_array_read_u32(in, block_number)) {
		return FALSE;
	}

	uint32_t data_len = 0U;
	if (!csm_axdr_rd_octetstring(in, &data_len)) {
		return FALSE;
	}

	uint32_t block_count = image_block_count(d);
	if ((*block_number >= block_count) || (block_count == 0U)) {
		return FALSE;
	}
	...
	if (*block_number == (block_count - 1U)) {
		...
		if (remainder != 0U) {
			expected_max = remainder;
		}
	}

	if ((data_len == 0U) || (data_len > expected_max)) {
		return FALSE;
	}
	return csm_array_reader_jump(in, data_len);
}

static csm_db_code image_dispatch(db_ic_inst_t *inst, db_ic_op_t op, uint8_t attr_id, uint8_t method_id, csm_array *in, csm_array *out) {
	...
	if (op == IC_OP_GET) {
		if (attr_id == 1U) {
			...
			if (d->transferred_blocks_len > 0U) {
				valid = valid && csm_array_write_buff(out, d->transferred_blocks_status, d->transferred_blocks_len);
			}
			return valid ? CSM_OK : CSM_ERR_BAD_ENCODING;
		} else if (attr_id == 6U) {
			int valid = csm_array_write_u8(out, AXDR_TAG_UNSIGNED32);
		...
		if (attr_id == 4U) {
			...
			if (!csm_array_read_u32(in, &d->block_size)) {
				return CSM_ERR_BAD_ENCODING;
			}
			return CSM_OK;
		} else if (attr_id == 7U) {
			...
			if (!csm_array_read_u8(in, &d->transfer_enabled)) {
				return CSM_ERR_BAD_ENCODING;
			}
			return CSM_OK;
		}
		...
		switch (method_id) {
			case 1U: {
				if ((d->transfer_status != IMG_STATUS_INITIALIZATION_OK) && (d->transfer_status != IMG_STATUS_INITIATED)) {
					return CSM_ERR_TEMPORARY_FAILURE;
				}
				uint32_t block_number = 0U;
				if (!image_read_block_transfer(in, d, &block_number)) {
					return CSM_ERR_BAD_ENCODING;
				}
			...
			case 2U:
				if (d->transfer_enabled == 0U) {
					return CSM_ERR_UNAUTHORIZED_ACCESS;
				}
			...
			case 5U: /* image_verify */
				if ((d->transfer_status != IMG_STATUS_INITIATED) || !image_all_blocks_transferred(d)) {
...
static const db_ic_class ic_image = {.class_id = 18, .name = "Image Transfer", .version = 0, .create = image_create, .dispatch = image_dispatch};
