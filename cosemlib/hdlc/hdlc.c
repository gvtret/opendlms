	...
	if (info_field_size > 0U) {
		switch (hdlc->type) {
			...
			case HDLC_PACKET_TYPE_UA: {
				if ((buf[0] == 0x81U) && (buf[1] == 0x80U) && ((buf[2] + 3U) == info_field_size)) {
					// Decode framing options
					uint8_t index = 3U;
					uint8_t number_of_tags = 0U;
					while (ret == HDLC_OK) {
						// Bounds check: need at least 2 bytes for tag + size
						if ((index + 2U) > info_field_size) {
							ret = HDLC_ERR_NEGO;
							break;
						}
...
static const uint8_t snrm_nego[] = {0x81U, 0x80U, 0x12U, 0x05U, 0x01U, 0x80U, 0x06U, 0x01U, 0x80U, 0x07U, 0x04U,
                                    0x00U, 0x00U, 0x00U, 0x01U, 0x08U, 0x04U, 0x00U, 0x00U, 0x00U, 0x07};
...
int hdlc_encode(hdlc_t *hdlc, uint8_t *buf, uint16_t size, uint8_t frame_type, const uint8_t *data, uint16_t data_size) {
	...
	if ((data_size > 0U) && (data != NULL)) {
		...
		// Bounds check: info field + 3 bytes overhead (7E + frame_type/length) + FCS(2) + 1
		if ((info_field_start + data_size + 3U) > size) {
			return -1;
		}
...
int hdlc_decode(hdlc_t *hdlc, const uint8_t *buf, uint16_t size) {
	...
	if (buf[0] == 0x7E) {
		...
		if (format == HDLC_FORMAT_TYPE) {
			...
			hdlc->frame_size =
			    hdlc_get_len(&buf[1]) + 2U;  // The value of the frame length subfield is the count of octets in the frame excluding the opening and
			                                 // closing frame flag sequences.

			if ((hdlc->frame_size >= HDLC_MIN_FRAME_SIZE) && (hdlc->frame_size <= size) && (buf[hdlc->frame_size - 1] == 0x7E)) {
