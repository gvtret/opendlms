...
void db_cosem_push_bind_trigger(const csm_obis_code *data_obis, const csm_obis_code *push_obis)
{
    ...
    CSM_LOG("[PUSH] Bound data OBIS %02X%02X%02X%02X%02X%02X to push OBIS %02X%02X%02X%02X%02X%02X",
            data_obis->A, data_obis->B, data_obis->C, data_obis->D, data_obis->E, data_obis->F,
            push_obis->A, push_obis->B, push_obis->C, push_obis->D, push_obis->E, push_obis->F);
}
