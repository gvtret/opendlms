...
void csm_print_obis(const csm_object_t *data) {
	CSM_TRACE(
	    "Class: %d, obis: %d.%d.%d.%d.%d.%d, id: %d\r\n", data->class_id, data->obis.A, data->obis.B, data->obis.C, data->obis.D, data->obis.E, data->obis.F,
	    data->id
	);
}
...
uint8_t csm_is_obis_equal(const csm_obis_code *first, const csm_obis_code *second) {
	uint8_t ret = FALSE;
	if ((first->A == second->A) && (first->B == second->B) && (first->C == second->C) && (first->D == second->D) && (first->E == second->E) &&
	    (first->F == second->F)) {
		ret = TRUE;
	}
