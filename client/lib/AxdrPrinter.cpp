...
//            = std::chrono::system_clock::to_time_t(std::chrono::system_clock::now());
        ...
        */
		...
		ss << clk.date.year << "-" << (int)clk.date.month << "-" << (int)clk.date.day << "T" << (int)clk.time.hour << ":" << (int)clk.time.minute << ":"
		   << (int)clk.time.second;
		dateTime = ss.str();
	}
	return dateTime;
...
void AxdrPrinter::Append(uint8_t type, uint32_t size, uint8_t *data) {
	...
	if ((type == AXDR_TAG_ARRAY) || (type == AXDR_TAG_STRUCTURE)) {
	...
	while (mLevels.size() > 0) {
		if (mLevels.back().counter >= mLevels.back().size) {
			...
			if ((prev_type == AXDR_TAG_ARRAY) || (prev_type == AXDR_TAG_STRUCTURE)) {
				PrintIndent();
