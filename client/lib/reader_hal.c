...
static uint8_t reader_system_title[CSM_DEF_APP_TITLE_SIZE] = {0x4DU, 0x4DU, 0x4DU, 0x00U, 0x00U, 0xBCU, 0x61U, 0x4EU};
...
uint8_t csm_hal_get_dedicated_key(uint8_t sap, uint8_t *buf, uint8_t buf_cap) {
	...
	if ((buf == NULL) || (buf_cap < 16U) || (idx >= READER_HAL_MAX_SAP) || (reader_ded_valid[idx] == 0U)) {
		return 0U;
	}
...
int csm_sys_gcm_init(uint8_t channel, uint8_t sap, csm_sec_key key_id, csm_sec_mode mode, const uint8_t *iv, const uint8_t *aad, uint32_t aad_len) {
	...
	mbedtls_gcm_init(&reader_gcm_ctx[channel]);
	{
		...
		if (mbedtls_gcm_setkey(&reader_gcm_ctx[channel], MBEDTLS_CIPHER_ID_AES, key, key_bits) != 0) {
			mbedtls_gcm_free(&reader_gcm_ctx[channel]);
			return 0;
		}
	}
...
int csm_sys_gcm_update(uint8_t channel, const uint8_t *plain, uint32_t plain_len, uint8_t *crypt) {
	if ((channel >= READER_HAL_MAX_CHANNELS) || (reader_gcm_active[channel] == 0U) || ((plain_len > 0U) && ((plain == NULL) || (crypt == NULL)))) {
		return 0;
	}
...
uint8_t csm_hal_get_random_u8(uint8_t min, uint8_t max) {
	...
	/* If at least one random byte was collected (value != 0 or attempt > 0),
     * use it rather than returning a fully deterministic fallback. */
	if (value != 0U || span > 1U) {
		return (uint8_t)(min + (value % span));
	}
