...
CosemClient::CosemClient(): mModemState(DISCONNECTED), mCosemState(CONNECT_HDLC), mReadIndex(0U), mMeterIndex(0U) {
}
				...
				do {
					...
					if (ret == HDLC_OK) {
						if (hdlc.type == HDLC_PACKET_TYPE_RR) {
							...
							if ((hdlc.segmentation == 0U) && (hdlc.poll_final == 1U)) {
								puts("Final packet\r\n");
...
bool HasGoodLlc(csm_array *array) {
	...
	if (valid && (llc1 == 0xE6U) && (llc2 == 0xE7U) && (llc3 == 0x00U)) {
		ret = true;
	}
	...
	if (digest_size > 0U) {
		...
		if (csm_axdr_wr_octetstring(&request.db_request.additional_data.data, &digest_stoc[0], digest_size)) {
			...
			if (result.success) {
				...
				if ((response.access_result == CSM_ACCESS_RESULT_SUCCESS) && (response.has_data == TRUE)) {
					// Get digest
					uint32_t size;
				...
				if (HasGoodLlc(&scratch_array)) {
					...
					if (csm_client_decode(&response, &scratch_array)) {
						...
						if (isResponseValid) {
							...
							if ((response.service == SVC_GET) || (response.service == SVC_ACTION)) {
								std::stringstream ss;
