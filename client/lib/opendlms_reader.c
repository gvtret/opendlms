...
static int reader_transport_send(void *ctx, uint8_t channel, const uint8_t *data, uint32_t len) {
	...
	int framed_len = csm_tcp_wrapper_frame(session->source_wport, session->dest_wport, data, len, framed, sizeof(framed));
	if (framed_len < 0) {
		return framed_len;
	}
...
static int reader_transport_recv(void *ctx, uint8_t channel, uint8_t *buf, uint32_t buf_size, uint32_t timeout_ms) {
	(void)channel;
	opendlms_reader_session_t *session = (opendlms_reader_session_t *)ctx;
	...
	if ((header_apdu_len == 0U) || (header_apdu_len > buf_size) || ((uint32_t)header_apdu_len + CSM_TCP_WRAPPER_LEN > sizeof(frame))) {
	...
	int rc = csm_tcp_wrapper_deframe(frame, (uint32_t)header_apdu_len + CSM_TCP_WRAPPER_LEN, &apdu, &apdu_len, &source_wport, &dest_wport);
...
static const csm_transport_ops reader_transport_ops = {reader_transport_open,  reader_transport_send,         reader_transport_recv,
                                                       reader_transport_close, reader_transport_is_connected, reader_transport_destroy};
...
static int reader_response_is_success(const csm_response *response) {
	...
	if (response->service == SVC_ACTION) {
		...
		return (response->access_result == CSM_ACCESS_RESULT_SUCCESS) || (response->access_result == CSM_ACCESS_RESULT_NOT_SET);
	}
...
int opendlms_reader_connect(opendlms_reader_session_t *session) {
	...
	if (!session->io.write || !session->io.read) {
		return -1;
	}
...
int opendlms_reader_get(opendlms_reader_session_t *session, uint16_t class_id, const csm_obis_code *obis, uint8_t attr_id, csm_response *response) {
	...
	int rc = csm_client_get_block(session->client, session->invoke_id++, class_id, obis, attr_id, resp_buf, sizeof(resp_buf));
	if (rc <= 0) {
		return -1;
	}
...
int opendlms_reader_set(
	...
	int rc = csm_client_set(session->client, session->invoke_id++, class_id, obis, attr_id, data, data_len, resp_buf, sizeof(resp_buf));
	if (rc <= 0) {
		return -1;
	}
...
int opendlms_reader_action(
	...
	int rc = csm_client_action(session->client, session->invoke_id++, class_id, obis, method_id, data, data_len, resp_buf, sizeof(resp_buf));
	if (rc <= 0) {
		return -1;
	}
